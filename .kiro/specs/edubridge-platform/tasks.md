...
## Rejalashtirilmasdan Yozilgan Kodlar Ro'yxati (Retrospektiv Tasklar)

> Bu bo'lim spec tasdiqlashdan **oldin** yozilgan, lekin tasks.md da aks ettirilmagan
> barcha implementatsiyalarni ro'yxatga oladi. Har bir task `[x]` (allaqachon bajarilgan)
> holatida belgilangan. Maqsad: kodni traceability bilan hujjatlashtirish.

---

- [x] 32. Gamification tizimi (rejalashtirilmasdan yozilgan)
  - [x] 32.1 Gamification backend moduli
    - `modules/gamification/` — XP, badge, streak, challenge, leaderboard, reward servislar
    - `gamification.service.ts`, `badge.service.ts`, `streak.service.ts`, `challenge.service.ts`, `leaderboard.service.ts`, `reward.service.ts`
    - `gamification-event.handler.ts` — lesson completion, submission grading, streak events
    - `gamification.cron.ts` — kunlik challenge reset, streak expiry
    - `gamification.controller.ts` — public API (XP, badges, leaderboard)
    - BullMQ workers: `gamification.processor.ts`, `gamification-outbox.processor.ts`
    - Repositories: `badge.repository.ts`, `challenge.repository.ts`, `gamification-profile.repository.ts`, `xp-event.repository.ts`
    - DB scripts: `gamification-seed.ts`, `add-test-xp.ts`, `sync-leaderboard.ts`
    - _Note: Req 20 (scalability) bilan bog'liq — student engagement. Frontend: `components/gamification/`_

  - [x] 32.2 Gamification frontend komponentlari
    - `components/gamification/achievements-dashboard.tsx` — foydalanuvchi yutuqlari
    - `components/gamification/daily-challenges-widget.tsx` — kunlik vazifalar widget
    - `components/gamification/gamification-onboarding.tsx` — onboarding gamification intro
    - `components/gamification/leaderboard-dashboard.tsx` — reyting jadvali
    - `components/gamification/rewards-dashboard.tsx` — mukofotlar ro'yxati
    - `components/dashboard/xp-badge.tsx` — XP ko'rsatish badge komponenti
    - `components/providers/gamification-toast-provider.tsx` — XP earned toast
    - Web sahifalar: `/achievements`, `/leaderboard`, `/rewards`
    - _Note: Student engagement funksionaliyat, spec da rejalashtirilmagan_

- [x] 33. Kengaytirilgan AI funksionaliyat (rejalashtirilmasdan yozilgan)
  - [x] 33.1 AI adapter arxitekturasi
    - `modules/ai/adapters/anthropic.adapter.ts` — Anthropic Claude adapter
    - `modules/ai/adapters/openai.adapter.ts` — OpenAI fallback adapter
    - `modules/ai/adapters/fake.adapter.ts` — test uchun fake adapter
    - `modules/ai/adapters/types.ts` — adapter interface/port
    - _Note: Req 13 (AI tutor) da faqat Claude belgilangan, OpenAI fallback qo'shimcha_

  - [x] 33.2 AI suhbat tarixi tizimi
    - `modules/ai/conversations/ai-conversations.controller.ts`
    - `modules/ai/conversations/ai-conversations.service.ts`
    - Foydalanuvchi AI suhbat tarixini saqlash va ko'rsatish
    - _Note: Req 13 da suhbat tarixi aniq belgilanmagan_

  - [x] 33.3 AI cost kalkulyator
    - `modules/ai/cost-calculator.ts`, `modules/ai/cost-calculator.spec.ts`
    - Token narxlari asosida real-time cost hisoblash
    - Model-specific pricing (Claude Haiku, Sonnet, Opus)
    - _Note: Req 13.4 da cost tracking bor, lekin kalkulyator alohida modul sifatida yozilmagan_

  - [x] 33.4 AI test generator
    - `components/ai/ai-test-generator.tsx` — AI yordamida test/savol generatsiyasi
    - O'qituvchi uchun avtomatik test savollari yaratish
    - _Note: Req 13 da faqat tutor mode va AI grading bor, test generator qo'shimcha_

  - [x] 33.5 AI grading panel
    - `components/ai/ai-grading-panel.tsx` — AI baholash natijalarini ko'rsatish paneli
    - `components/ai/ai-chat-page.tsx` — to'liq ekran AI chat sahifasi
    - `components/ai/ai-chat-widget.tsx` — float AI chat widget
    - `/ai-chat` sahifasi (dashboard-full layout)
    - _Note: Req 12.4, 13 bilan bog'liq, lekin alohida sahifa va panel rejalashtirilmagan_

- [x] 34. Kengaytirilgan Live streaming funksiyalar (rejalashtirilmasdan yozilgan)
  - [x] 34.1 LiveKit integratsiyasi (mediasoup alternativasi)
    - `modules/live/sfu/livekit.service.ts` — LiveKit SDK integratsiyasi
    - `modules/live-stream/` — alohida live-stream modul (live module dan tashqari)
    - `apps/web/node_modules/livekit-client` — LiveKit client library
    - _Note: Req 9 da faqat mediasoup SFU belgilangan; LiveKit qo'shimcha yoki muqobil_

  - [x] 34.2 Whiteboard funksiyaligi
    - `components/live-room/Whiteboard.tsx` — real-time whiteboard (tldraw asosida)
    - `apps/web/node_modules/tldraw` — tldraw library
    - `components/live-room/LiveRoom.tsx` — to'liq live room komponenti
    - _Note: Req 9 da whiteboard belgilanmagan_

  - [x] 34.3 Screen capture himoyasi
    - `hooks/use-content-protection.ts` — kontent himoya hook
    - `hooks/use-screen-capture-detection.ts` — ekran yozib olish aniqlash
    - `components/lesson/user-watermark.tsx` — foydalanuvchi watermark overlay
    - `components/ui/protected-media.tsx` — himoyalangan media player wrapper
    - _Note: Req 8-9 da content protection rejalashtirilmagan_

  - [x] 34.4 Live room boshqaruv komponentlari
    - `components/live/live-control-bar.tsx` — mute/camera/share screen tugmalari
    - `components/live/live-session-join.tsx` — join session UI
    - `components/live/live-sidebar.tsx` — chat va ishtirokchilar panel
    - `components/live/live-top-bar.tsx` — live session header
    - `components/live/tabs/` — tab-based live UI
    - _Note: Req 9 frontend rejalashtirilmagan, faqat backend spec da bor_

- [x] 35. Frontend marketing va landing sahifalar (rejalashtirilmasdan yozilgan)
  - [x] 35.1 Marketing landing page komponentlari
    - `components/landing/hero.tsx`, `hero-v2.tsx` — hero section (2 variant)
    - `components/landing/features-bento.tsx` — 7 feature bento grid
    - `components/landing/stats.tsx`, `animated-counter.tsx` — statistika animatsiya
    - `components/landing/testimonials.tsx` — izohlar bo'limi
    - `components/landing/pricing.tsx` — narxlar bo'limi
    - `components/landing/faq.tsx` — savol-javob
    - `components/landing/footer.tsx`, `header.tsx` — navigatsiya
    - `components/landing/ai-section.tsx`, `live-showcase.tsx` — feature showcase
    - `components/landing/aurora-backdrop.tsx`, `cursor-spotlight.tsx`, `light-beam.tsx` — animatsiya effektlar
    - `components/landing/logos-strip.tsx`, `trust-marquee.tsx` — trust signallar
    - _Note: web-design.md da belgilangan (Faza A bajarilgan), lekin tasks.md da alohida task yo'q_

  - [x] 35.2 Marketing komponentlar kutubxonasi
    - `components/marketing/animated.tsx` — Framer Motion wrapper komponentlar
    - `components/marketing/audience-toggle.tsx` — teacher/student toggle
    - `components/marketing/hero-visual.tsx`, `hero-v2.tsx` — hero vizual
    - `components/marketing/cta-button.tsx`, `enroll-cta.tsx` — CTA tugmalar
    - `components/marketing/discovery-cards.tsx`, `discovery-skeletons.tsx` — kurs kartalari
    - `components/marketing/role-switcher.tsx` — rol almashish
    - `components/marketing/search-controls.tsx` — qidiruv filtrlari
    - `components/marketing/teacher-dm-button.tsx` — DM tugmasi
    - `components/marketing/effects/`, `sections/` — vizual effektlar va bo'limlar
    - _Note: web-design.md da belgilangan lekin tasks.md da yo'q_

  - [x] 35.3 Auth sahifalari va komponentlar
    - `components/auth/aurora-auth-layout.tsx` — aurora effektli auth layout
    - `components/auth/login-form.tsx`, `register-form.tsx`, `forgot-password-form.tsx`, `reset-password-form.tsx`
    - `components/auth/password-strength.tsx` — parol kuchi indikatori
    - `components/auth/role-selector.tsx` — teacher/student tanlov
    - `components/auth/verify-email-client.tsx` — email tasdiqlash client component
    - Auth sahifalar: `/login`, `/register`, `/verify-email`, `/forgot-password`, `/reset-password`, `/signup`, `/password-reset`
    - _Note: web-design.md Faza B belgilangan, tasks.md da yo'q_

- [x] 36. Dashboard frontend sahifalar va komponentlar (rejalashtirilmasdan yozilgan)
  - [x] 36.1 Teacher dashboard komponentlari
    - `components/dashboard/teacher-dashboard.tsx` — asosiy teacher dashboard
    - `components/dashboard/teacher-analytics.tsx` — analitika komponenti
    - `components/dashboard/course-grid.tsx`, `course-form.tsx` — kurs boshqaruvi
    - `components/dashboard/group-form.tsx`, `group-delete-button.tsx` — guruh boshqaruvi
    - `components/dashboard/lesson-form.tsx`, `lesson-row.tsx`, `lesson-materials.tsx` — dars boshqaruvi
    - `components/dashboard/module-toggle-panel.tsx` — homework module toggles
    - `components/dashboard/enrollment-requests.tsx` — enrollment so'rovlari
    - `components/dashboard/invite-link-button.tsx` — invite link generatsiya
    - `components/dashboard/onboarding-wizard.tsx` — specialty onboarding
    - `components/dashboard/sidebar.tsx`, `top-nav.tsx` — navigatsiya
    - `components/dashboard/student-list.tsx` — guruh talabalari
    - _Note: web-design.md Faza E-F belgilangan, tasks.md da yo'q_

  - [x] 36.2 Student dashboard komponentlari
    - `components/student/student-dashboard.tsx` — student asosiy sahifasi
    - `components/student/student-courses.tsx` — ro'yxatdan o'tilgan kurslar
    - `components/student/student-schedule.tsx` — jadval ko'rinishi
    - `components/student/group-detail.tsx` — guruh tafsilotlari
    - Student sahifalar: `/dashboard`, `/my-courses`, `/assignments`, `/submissions`, `/achievements`, `/leaderboard`, `/rewards`
    - _Note: web-design.md Faza H belgilangan, tasks.md da yo'q_

  - [x] 36.3 Lesson player komponentlari
    - `components/lesson/hls-player.tsx` — HLS.js video player
    - `components/lesson/lesson-player.tsx` — to'liq lesson player (video + notes + AI tutor panel)
    - `components/lesson/user-watermark.tsx` — DRM watermark
    - Sahifalar: `/lessons/{id}`, `/lessons/{id}/homework`
    - _Note: Req 8 (HLS) bor lekin frontend task yo'q_

  - [x] 36.4 Homework frontend komponentlari
    - `components/homework/assignment-builder.tsx` — drag-and-drop assignment yaratish
    - `components/homework/submission-editor.tsx` — autosave submission editor
    - `components/homework/homework-submission-form.tsx`, `homework-assignment-view.tsx`
    - `components/homework/teacher-grading-detail.tsx`, `teacher-grading-queue.tsx`, `teacher-submissions-list.tsx`
    - `components/homework/student-assignment-detail.tsx`, `homework-overview.tsx`
    - `components/homework/module-runtimes/` — modul-specific UI (writing, reading, listening)
    - `components/homework/module-runner-reading.tsx`, `module-runner-writing.tsx`
    - _Note: Req 12 backend bor, lekin frontend komponentlar tasks.md da rejalashtirilmagan_

  - [x] 36.5 Notifications va Messages komponentlari
    - `components/notifications/notification-bell.tsx` — header notification bell
    - `components/notifications/notification-inbox.tsx`, `notifications-list.tsx`
    - `components/notifications/notification-filters.tsx`, `notification-item.tsx`
    - `components/notifications/notification-preferences.tsx` — sozlamalar
    - `components/messages/conversation-list.tsx`, `message-thread.tsx`, `new-message-dialog.tsx`
    - Hooks: `use-notifications-inbox.ts`, `use-unread-notifications-count.ts`, `use-unread-dm-count.ts`
    - `/notifications` va `/messages/{userId}` sahifalari
    - _Note: Req 15-16 backend bor, lekin frontend tasks.md da yo'q_

  - [x] 36.6 Settings sahifalari
    - `components/settings/profile-settings.tsx`, `settings-layout.tsx`
    - `/settings/profile`, `/settings/notifications` sahifalari
    - _Note: web-design.md Faza L belgilangan, tasks.md da yo'q_

- [x] 37. Kengaytirilgan infratuzilma va DevOps (rejalashtirilmasdan yozilgan)
  - [x] 37.1 Kubernetes (K8s) konfiguratsiyasi
    - `infra/k8s/base/namespace.yaml` — K8s namespace
    - `infra/k8s/base/network-policies.yaml` — pod-to-pod tarmoq qoidalari
    - `infra/k8s/base/pod-security-policy.yaml` — Pod Security Standards
    - `infra/k8s/argocd/application.yaml` — ArgoCD GitOps konfiguratsiyasi
    - `infra/k8s/admission/image-policy.yaml` — image admission controller
    - `infra/k8s/drift-detection/drift-alert.yaml` — infrastructure drift alertlari
    - `infra/k8s/vault/external-secrets.yaml`, `vault-agent-config.yaml` — HashiCorp Vault
    - _Note: Req 26 (CI/CD) da Docker Compose bor, lekin K8s manifests rejalashtirilmagan_

  - [x] 37.2 Cloudflare WAF qoidalari
    - `infra/cloudflare/waf-rules.md` — WAF qoidalari dokumentatsiyasi
    - _Note: Req 21 (security) da WAF qayd etilgan lekin infra konfiguratsiyasi task sifatida yo'q_

  - [x] 37.3 Docker production Dockerfile lar
    - `infra/docker/Dockerfile.api` — NestJS API production image
    - `infra/docker/Dockerfile.web` — Next.js production image
    - _Note: Task 1.6 da faqat docker-compose.yml bor, production Dockerfile rejalashtirilmagan_

  - [x] 37.4 Security compliance hujjatlar
    - `infra/security/compliance/soc2-controls.md` — SOC 2 nazorat mexanizmlari
    - `infra/security/compliance/security-headers-checklist.md` — security headers checklist
    - `infra/security/tls.md` — TLS konfiguratsiya qo'llanma
    - `infra/security/backup/run-backup.sh` — backup automation script
    - `infra/security/supply-chain/README.md`, `infrastructure/README.md`
    - _Note: Req 21 security ga bog'liq hujjatlar, tasks.md da yo'q_

  - [x] 37.5 CI/CD pipeline kengaytmalar
    - `.github/workflows/sast-dast.yml` — SAST/DAST pipeline (Semgrep + OWASP ZAP)
    - `.github/workflows/security-scan.yml` — Trivy + Snyk container scanning
    - `.github/dependabot.yml` — avtomatik dependency updates
    - `.github/zap-rules.tsv` — ZAP scanner qoidalari
    - _Note: Task 1.6 da faqat `ci.yml` bor, security CI pipelines rejalashtirilmagan_

  - [x] 37.6 Load testing va monitoring skriptlar
    - `scripts/load-test-sim.js` — load test simulatsiya skripti
    - `scripts/web-load-test.js` — web app load test
    - `scripts/verify-session.sh` — session tekshirish skript
    - `scripts/dev-no-mobile.sh` — mobile app siz dev server
    - `apps/api/scripts/backup-encrypt.ts`, `backup-decrypt.ts`, `backup-cli.spec.ts`
    - `apps/api/scripts/publish-demo-lessons.ts`, `upload-demo-direct.ts`, `smtp-test.js`
    - _Note: Req 20 (performance) bor, lekin test skriptlar tasks.md da rejalashtirilmagan_

- [x] 38. Backend kengaytirilgan modullar (rejalashtirilmasdan yozilgan)
  - [x] 38.1 Reports va CSV export moduli
    - `modules/reports/reports.service.ts`, `reports-admin.controller.ts`, `reports-teacher.controller.ts`
    - `modules/reports/csv-stream.ts` — streaming CSV export
    - `modules/reports/csv-columns.ts` — export column definitsiyalar
    - Student progress, to'lov va submission CSV eksporti
    - _Note: Task 23.3 da "data export" bor, lekin alohida reports modul spec da yo'q_

  - [x] 38.2 MFA moduli (alohida, security moduldan tashqari)
    - `modules/mfa/mfa.service.ts`, `mfa.controller.ts`, `mfa.module.ts`
    - `modules/mfa/mfa.encryption.ts` — TOTP secret encryption
    - TOTP + WebAuthn DTO lar: `totp.dto.ts`, `webauthn.dto.ts`, `mfa-challenge.dto.ts`, `backup-codes.dto.ts`
    - _Note: Task 29.1 da security/mfa bor, lekin modules/mfa alohida modul qo'shimcha_

  - [x] 38.3 Users moduli
    - `modules/users/users.service.ts`, `users.controller.ts`, `users.module.ts`
    - Profile update, username o'zgartirish, user qidirish
    - DTOs: `update-profile.dto.ts`, `update-username.dto.ts`, `search-users.dto.ts`
    - _Note: Auth modulida `User` bor, lekin alohida `users` modul rejalashtirilmagan_

  - [x] 38.4 Click va UzumPay to'lov integrasiyalari
    - `modules/billing/click/click.controller.ts` — Click to'lov tizimi controller
    - `modules/billing/uzumpay/uzumpay.controller.ts` — UzumPay controller
    - _Note: Req 4 da faqat Payme belgilangan; Click va UzumPay qo'shimcha to'lov tizimlari_

  - [x] 38.5 Schedule moduli (alohida)
    - `modules/schedule/schedule.service.ts`, `schedule.controller.ts`, `schedule.module.ts`
    - `modules/schedule/repositories/schedule.repository.ts`
    - ScheduleException management, RRULE parsing
    - _Note: Task 14.1 da schedule catalog modulida bor, lekin alohida `schedule` modul rejalashtirilmagan_

  - [x] 38.6 Metrics va observability infratuzilma
    - `infra/metrics/metrics.service.ts`, `metrics.controller.ts`, `metrics.interceptor.ts`
    - Prometheus-compatible `/metrics` endpoint (prom-client)
    - Request count, latency histogram, error rate gauge
    - `infra/sentry/` — Sentry error tracking adapter (port pattern)
    - `infra/prisma/slow-query.middleware.ts` — DB slow query logging
    - `infra/prisma/prisma-router.ts` — primary/replica routing
    - `common/observability/structured-logger.ts`, `trace-context.ts`, `bullmq-trace.ts`
    - _Note: Task 24.3 da "Observability sozlash" bor, lekin metrics endpoint va Sentry adapter rejalashtirilmagan_

  - [x] 38.7 Cache infratuzilma
    - `infra/cache/cache.service.ts`, `cache.module.ts`
    - HTTP response caching interceptor: `cache-response.interceptor.ts`, `http-cache.interceptor.ts`, `response-cache.interceptor.ts`
    - _Note: Task 24.2 da Redis cache bor, lekin cache infra moduli va HTTP caching interceptors alohida rejalashtirilmagan_

  - [x] 38.8 Threat protection moduli (alohida)
    - `modules/threat-protection/threat-protection.service.ts`, `controller.ts`, `guard.ts`, `module.ts`
    - WAF + DDoS + suspicious request detection pipeline
    - Geo-blocker, tiered rate limiter, bot detection birlashtirilgan
    - _Note: Task 27 da bu funksiyalar mavjud, lekin alohida `threat-protection` modul qo'shimcha_

  - [x] 38.9 Homework runtimes arxitekturasi
    - `modules/homework/runtimes/registry.ts` — modul type → runtime mapping
    - `modules/homework/runtimes/reading.runtime.ts`, `writing.runtime.ts` — runtime implementatsiyalar
    - `modules/homework/runtimes/types.ts` — runtime interface/port
    - _Note: Task 18.3-18.4 da writing va reading modullar bor, lekin runtime registry pattern rejalashtirilmagan_

  - [x] 38.10 API client paketi (packages/api-client)
    - `packages/api-client/src/client.ts` — base HTTP client
    - `packages/api-client/src/sdk.ts` — typed API SDK
    - `packages/api-client/src/jwt.ts` — token management
    - `packages/api-client/src/cert-pinning.ts` — certificate pinning
    - `packages/api-client/src/endpoints/` — endpoint-specific clients
    - _Note: Task 22.1 da `packages/api-client` "shared API client" bor, lekin cert pinning va full SDK rejalashtirilmagan_

- [x] 39. Boshqa rejalashtirilmagan fayllar va utilitylar
  - [x] 39.1 Web app hooks va lib fayllar
    - `hooks/use-classroom.ts` — sinf xonasi state management
    - `hooks/use-live-session.ts` — live session state
    - `hooks/use-pending-homework-count.ts` — pending homework badge
    - `hooks/use-pending-requests.ts` — enrollment request badge
    - `lib/api/` — endpoint-specific API client wrappers
    - `lib/validation/` — frontend validation schemas
    - `lib/discovery-api.ts`, `lib/dm-api.ts`, `lib/homework-api.ts`, `lib/notifications-api.ts`
    - `lib/gamification-constants.ts` — XP, level, badge constants
    - `app/api/media/proxy/` — Next.js media proxy API route
    - _Note: Ular mavjud feature larni bog'laydi lekin tasklar sifatida rejalashtirilmagan_

  - [x] 39.2 Mobile app kengaytirilgan funksiyalar
    - `apps/mobile/lib/cache/` — SQLite offline cache
    - `apps/mobile/lib/notifications/` — push notification handlers
    - `apps/mobile/lib/sync/` — background sync logic
    - `apps/mobile/lib/api/` — mobile-specific API wrapper
    - `apps/mobile/app/(tabs)/` — tab navigation screens
    - `apps/mobile/app/(auth)/` — auth flow screens
    - `apps/mobile/services/lessons.ts`, `services/notifications.ts`
    - `apps/mobile/security/` — ATS (iOS) va Android network security config
    - _Note: Task 22.1-22.2 da skeleton bor, lekin full implementation rejalashtirilmagan_

  - [x] 39.3 Database migrations va scripts
    - `packages/db/prisma/migrations/` — barcha DB migration fayllari
    - `packages/db/scripts/activate-pending-users.ts` — pending user activation skript
    - `packages/db/scripts/load-test-gen.ts` — load test data generator
    - `apps/api/scripts/publish-demo-lessons.ts` — demo content seed
    - `create-test-users.ts` — test user creation (root darajasida)
    - `convert_to_sqlite.py` — SQLite konversiya utility
    - _Note: Task 1.4 da seed script bor, lekin alohida utility skriptlar rejalashtirilmagan_

  - [x] 39.4 Branding va i18n
    - `packages/i18n/locales/uz/`, `ru/`, `en/` — to'liq tarjima fayllari (3 til)
    - `apps/mobile/theme/colors.ts` — mobile rang tizimi
    - `apps/web/tailwind.config.ts` — Pomelo/Forest/Gold rang tizimi
    - `apps/mobile/lib/i18n.ts` — mobile i18n setup
    - _Note: Task 1.3 da i18n konfiguratsiyasi bor, lekin to'liq tarjima fayllari rejalashtirilmagan_

---

## Yangilangan Notes

- Tasklar 32-39 — spec tasdiqlashdan oldin yozilgan kodlarni hujjatlashtiradi
- Bu tasklar allaqachon `[x]` (bajarilgan) holatida — ular hozirda mavjud
- Ular traceability va audit maqsadida qo'shilgan
- **Gamification** (32) — katta qo'shimcha feature, yangi requirements qo'shish tavsiya etiladi
- **LiveKit** (34.1) — mediasoup bilan parallel, integration test kerak
- **Click/UzumPay** (38.4) — Payme bilan birga to'lov tizimi, test coverage kerak
- **Whiteboard** (34.2) — tldraw integratsiyasi, real-time sync test kerak
