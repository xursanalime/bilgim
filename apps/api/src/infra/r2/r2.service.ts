...
/**
 * Result of {@link R2Service.getObjectStream}. Carries the readable body
 * plus the storage-side metadata the API needs to proxy bytes (and Range
 * responses) back to the client without exposing the object key/URL.
 */
export interface R2ObjectStream {
  ...
  contentRange?: string;
  acceptRanges: string;
  /** True when R2 satisfied a Range request with a partial (206) body. */
  isPartial: boolean;
}
...
export class R2Service implements OnModuleInit, OnModuleDestroy {
  ...
  /** When 'local', all object I/O is served from the local filesystem. */
  ...
  constructor(
    ...
    this.driver =
      (this.config.get<string>('STORAGE_DRIVER') ?? 'r2').toLowerCase() === 'local'
        ? 'local'
        : 'r2';
    ...
    this.bucket =
      this.config.get<string>('R2_BUCKET_NAME') ?? 'bilgim-media';
  ...
  async onModuleInit(): Promise<void> {
    if (this.driver === 'local') {
      this.logger.log('Storage driver: LOCAL filesystem (STORAGE_DRIVER=local)');
      return;
    }
  ...
  async createMultipartUpload(
    ...
    if (this.driver === 'local') {
      return this.local.createMultipartUpload(key, contentType);
    }
  ...
  async signUploadPart(
    ...
    if (this.driver === 'local') {
      return this.local.signUploadPart(key, uploadId, partNumber, ttl);
    }
  ...
  async signUploadPartBatch(
    ...
    if (this.driver === 'local') {
      ...
      return this.local.signUploadPartBatch(key, uploadId, partsCount, ttl);
    }
  ...
  async completeMultipartUpload(
    ...
    if (this.driver === 'local') {
      return this.local.completeMultipartUpload(key, uploadId, parts);
    }
  ...
  async abortMultipartUpload(key: string, uploadId: string): Promise<void> {
    if (this.driver === 'local') {
      return this.local.abortMultipartUpload(key, uploadId);
    }
    try {
  ...
  async signObjectGet(
    ...
    if (this.driver === 'local') {
      return this.local.signObjectGet(key, ttl);
    }
  ...
  async getObjectReadStream(key: string): Promise<Readable> {
    if (this.driver === 'local') {
      return this.local.getObjectReadStream(key);
    }
    const response = await this.client.send(
      new GetObjectCommand({ Bucket: this.bucket, Key: key }),
    );
  ...
  /**
   * Open a byte stream for an object, optionally honouring an HTTP
   * `Range` header. Used by the protected-file streaming endpoint
   * (content-protection-backend Req 4.1) so the API can proxy bytes to
   * an entitled learner WITHOUT ever handing them a directly-addressable
   * (signed) storage URL.
   *
   * When `range` is supplied (e.g. `bytes=0-1023`) R2 returns a partial
   * `206` body and echoes `Content-Range`; we surface that metadata so
   * the controller can mirror the correct status + headers back to the
   * client. The underlying R2/S3 object key never leaves this layer.
   */
  async getObjectStream(
    key: string,
    range?: string,
  ): Promise<R2ObjectStream> {
    if (this.driver === 'local') {
      return this.local.getObjectStream(key, range);
    }
    ...
    const result: R2ObjectStream = {
      ...
      // R2 only emits Content-Range for a satisfied partial request.
      isPartial: Boolean(range) && Boolean(object.ContentRange),
    };
  ...
  async putObject(
    ...
    if (this.driver === 'local') {
      return this.local.putObject(key, body, contentType);
    }
    await this.client.send(
