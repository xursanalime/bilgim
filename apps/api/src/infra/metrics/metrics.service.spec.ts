 ...
 *   - edubridge_notification_failed_total      (Counter, business — terminal failures)
 *   - payme_transitions_total                  (Counter, business)
 *   - outbox_pending_events                    (Gauge, USE)
 *   - edubridge_outbox_pending                 (Gauge alias under edubridge_ namespace)
 *   - ai_calls_total                           (Counter, business)
 *   - edubridge_ai_call_duration_seconds       (Histogram, latency)
 */
describe('MetricsService', () => {
  ...
  it('exposes all required custom metric names in Prometheus exposition format', async () => {
    ...
    expect(body).toContain('# TYPE edubridge_notification_failed_total counter');
    expect(body).toContain('# TYPE payme_transitions_total counter');
    expect(body).toContain('# TYPE outbox_pending_events gauge');
    expect(body).toContain('# TYPE edubridge_outbox_pending gauge');
    expect(body).toContain('# TYPE ai_calls_total counter');
    expect(body).toContain('# TYPE edubridge_ai_call_duration_seconds histogram');
    ...
    expect(body).toMatch(
      /edubridge_notification_failed_total\{[^}]*kind="LESSON_PUBLISHED"[^}]*channel="EMAIL"[^}]*reason="upstream_5xx"[^}]*\}/,
    );
    expect(body).toMatch(
    ...
    expect(body).toMatch(
      /edubridge_ai_call_duration_seconds_count\{[^}]*intent="TUTOR_EXPLAIN"[^}]*provider="claude-3-5-sonnet-latest"[^}]*\}\s+1/,
    );
    expect(body).toMatch(/outbox_pending_events(?:\{[^}]*\})?\s+7/);
    expect(body).toMatch(/edubridge_outbox_pending(?:\{[^}]*\})?\s+7/);
  });
  ...
  it('observeAiCallDuration ignores non-finite latencies', async () => {
    ...
    expect(body).toMatch(
      /edubridge_ai_call_duration_seconds_count\{[^}]*intent="TUTOR_EXPLAIN"[^}]*provider="claude"[^}]*\}\s+1/,
    );
  });
});
