...
const TOPIC_TO_QUEUE: Record<string, string | readonly string[]> = {
  ...
  // Also feeds GamificationOutboxProcessor (first-login XP + streak).
  // See the array-fan-out note on 'live.started' below for why this
  // can't just be one processor listening on NOTIFICATION_FANOUT.
  'auth.login': [QUEUE_NAMES.NOTIFICATION_FANOUT, QUEUE_NAMES.GAMIFICATION_OUTBOX],
  ...
  // Homework events. `submission.submitted` and `homework.graded` each
  // feed both the student/teacher notification AND a gamification XP
  // award — two independent consumers, so both queues must get the
  // job (a single NOTIFICATION_FANOUT queue would only deliver it to
  // whichever @Processor happened to win that job, silently dropping
  // the other side's effect).
  'submission.created': QUEUE_NAMES.AI_GRADING,
  'submission.submitted': [
    QUEUE_NAMES.NOTIFICATION_FANOUT,
    QUEUE_NAMES.GAMIFICATION_OUTBOX,
  ],
  ...
  // AI & Catalog — gamification-only today (no NotificationFanoutProcessor
  // case for either topic), but routed straight to GAMIFICATION now that
  // it has its own queue rather than sharing NOTIFICATION_FANOUT.
  'ai.tutor_used': QUEUE_NAMES.GAMIFICATION_OUTBOX,
  'lesson.viewed': QUEUE_NAMES.GAMIFICATION_OUTBOX,
  ...
  implements OnModuleInit, OnModuleDestroy, OutboxDispatcherHealth
{
  ...
  constructor(
    ...
    @InjectQueue(QUEUE_NAMES.GAMIFICATION_OUTBOX) private readonly gamificationOutboxQueue: Queue,
  ...
  private getQueue(queueName: string | undefined): Queue | null {
    ...
    const queueMap: Record<string, Queue> = {
      ...
      [QUEUE_NAMES.GAMIFICATION_OUTBOX]: this.gamificationOutboxQueue,
    };
