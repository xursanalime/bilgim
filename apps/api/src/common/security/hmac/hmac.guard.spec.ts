...
describe('HmacGuard', () => {
  ...
  /**
   * `secret` is deliberately not a defaulted parameter: `makeConfig(undefined)`
   * would trigger the default and hand back the real secret, so the
   * "secret is missing" test silently exercised the happy path and expected
   * SIGNATURE_MISCONFIGURED while the guard produced INVALID_SIGNATURE.
   */
  function makeConfig(...args: [] | [string | undefined]) {
