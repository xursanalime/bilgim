...
describe('sanitizeHtml (Task 29.4 — XSS prevention)', () => {
  ...
  describe('safe content survives', () => {
    it('keeps allowed tags intact', () => {
      const input =
        '<p>Hello <b>world</b>, <i>welcome</i> to <em>EduBridge</em></p>' +
    ...
    it('preserves https / http / mailto anchors', () => {
      const out = sanitizeHtml(
        '<a href="https://edubridge.uz">site</a>' +
          '<a href="http://example.com">x</a>' +
          '<a href="mailto:hi@edubridge.uz">contact</a>',
      );
      expect(out).toMatch(/href="https:\/\/edubridge\.uz"/);
      expect(out).toMatch(/href="http:\/\/example\.com"/);
      expect(out).toMatch(/href="mailto:hi@edubridge\.uz"/);
    });
