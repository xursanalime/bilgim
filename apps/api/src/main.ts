...
async function bootstrap() {
  ...
  // Redis-backed Socket.io adapter — without this, `server.to(room).emit()`
  // in LiveChatGateway/ChatGateway only reaches sockets on the same pod.
  // Must be set before any gateway's server is created (i.e. before
  // `app.init()`/`app.listen()`), since Nest builds each gateway's Server
  // instance eagerly during module instantiation.
