...
describe('VaultKmsProvider — getActiveKey', () => {
  it('reads the latest version and exports its bytes', async () => {
    const fetcher = buildFetcher({
      ...
      '/v1/transit/export/encryption-key/bilgim': {
        ok: true,
        body: {
          data: {
    ...
    expect(active.id).toBe('bilgim:2');
  ...
  it('issues a POST to /rotate when createKey is called', async () => {
    ...
    const fetcher: VaultHttpFetcher = jest.fn(async (url, init) => {
      ...
      if (url.includes('/keys/bilgim') && !url.includes('rotate')) {
        return {
          ok: true,
          status: 200,
          json: async () => ({
    ...
    const provider = new VaultKmsProvider(
      { addr: 'https://vault.example', token: 'TOKEN', keyName: 'bilgim' },
      fetcher,
    );
  ...
  it('returns null for an id that does not match the configured keyName', async () => {
    const fetcher = buildFetcher({
      ...
      '/v1/transit/export/encryption-key/bilgim': {
        ok: true,
        body: {
          data: { keys: { '1': TRANSIT_KEY_BASE64 }, name: 'bilgim' },
        },
      },
    });
    const provider = new VaultKmsProvider(
    ...
    expect(await provider.getKeyById('bilgim:99')).toBeNull();
    const found = await provider.getKeyById('bilgim:1');
