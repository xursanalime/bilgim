...
export class SubmissionService {
  ...
  async submit(
    ...
    // Muddatdan o'tib topshirishni bloklash
    const dueAt = submission.assignment.dueAt;
    if (dueAt && dueAt.getTime() < Date.now()) {
      throw new ConflictException({
        code: 'SUBMISSION_DEADLINE_PASSED',
        message: 'Uy ishi muddati tugagan. Topshirish qabul qilinmaydi.',
      });
    }
   ...
   * - For TEACHER: submissions with status SUBMITTED or IN_REVIEW.
  ...
  async getPendingCount(actor: SubmissionActor): Promise<{ count: number }> {
    if (actor.role === 'TEACHER') {
      const count = await this.prisma.submission.count({
        where: {
          assignment: { lesson: { group: { course: { teacherId: actor.userId } } } },
