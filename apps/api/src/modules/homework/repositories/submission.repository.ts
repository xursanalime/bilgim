...
export class SubmissionRepository {
  ...
  /**
   * SUBMITTED, IN_REVIEW, or already-GRADED → GRADED. Sets `score` and
   * `gradedAt = NOW()`. Allowing GRADED as a source status lets a teacher
   * revise a grade they already gave, not just grade it once.
   */
  async transitionToGraded(
    ...
    const client = tx ?? this.prisma;
    const result = await client.submission.updateMany({
      where: { id, status: { in: ['SUBMITTED', 'IN_REVIEW', 'GRADED'] } },
