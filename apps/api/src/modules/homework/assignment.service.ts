...
import { isLanguageModuleType } from './language-module-types';
...
export class AssignmentService {
  ...
  async createForLesson(
    ...
    this.assertLanguageModuleTypes(dto.modules);
  ...
  /**
   * Restrict assignment creation to the eight English
   * `Language_Module_Type` values (English-Only Platform Refocus,
   * Req 6.1, 6.3). Any `Legacy_Module_Type` (MULTIPLE_CHOICE, GAP_FILL,
   * MATCHING, DRAG_DROP, PROJECT_SUBMISSION, CASE_STUDY, MARKETING_COPY,
   * AUDIENCE_ANALYSIS, CONTENT_CALENDAR, MATH_*, CODE_*) is rejected with
   * a 400 `MODULE_TYPE_NOT_SUPPORTED` error.
   *
   * This is a server-authoritative gate independent of client input and
   * runs before the GroupModule enablement check. It only governs NEW
   * module selections (create + module replacement on update); existing
   * assignments and their submissions are never touched, so legacy
   * historical data is preserved (Req 6 / Migration non-destructive
   * guarantee).
   *
   * All offending types are reported at once so the assignment-builder
   * UI can surface a single error.
   */
  private assertLanguageModuleTypes(
    modules: Array<{ type: HomeworkModuleType }>,
  ): void {
    if (modules.length === 0) return;

    const unsupported = new Set<HomeworkModuleType>();
    for (const m of modules) {
      if (!isLanguageModuleType(m.type)) unsupported.add(m.type);
    }
    ...
    if (unsupported.size > 0) {
      throw new BadRequestException({
        ...
        message:
          'These module types are no longer supported. Assignments may ' +
          'only use the eight English language skills (WRITING, READING, ' +
          'LISTENING, GRAMMAR, SPELLING, VOCABULARY, SPEAKING, ' +
          `PRONUNCIATION): ${Array.from(unsupported).join(', ')}`,
        details: { unsupported: Array.from(unsupported) },
      });
    }
  }
