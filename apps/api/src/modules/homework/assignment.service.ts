 ...
 *
 * STUDENT is read-only: the two read paths (`getById`, `listForLesson`)
 * authorize a student through an APPROVED enrollment in the lesson's group
 * and hide unpublished drafts. Every mutating path rejects STUDENT.
 */
export type AssignmentActorRole = 'TEACHER' | 'ADMIN' | 'STUDENT';
...
type AssignmentWithModules = Assignment & {
  modules: AssignmentModule[];
  /** Only populated by `getById` — other read/write paths don't need it. */
  attachments?: Array<{
    id: string;
    assetId: string;
    kind: string;
...
export class AssignmentService {
  ...
  async getById(
    ...
    if (actor.role === 'STUDENT') {
      await this.assertEnrolledStudent(actor, assignment.lesson.group.id);
      // A draft is invisible to students — 404 rather than 403 so the
      // response cannot be used to probe for unpublished homework.
    ...
    return {
      ...
      attachments: assignment.attachments.map((a) => ({
        ...a,
        asset: { ...a.asset, bytes: a.asset.bytes == null ? null : Number(a.asset.bytes) },
      })),
    } as AssignmentWithModules;
  }
  ...
  async listForLesson(
    ...
    if (actor.role === 'STUDENT') {
      await this.assertEnrolledStudent(actor, ownership.groupId);
      const all = await this.assignmentRepository.listByLesson(lessonId);
  ...
  async update(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // `existing`, `updateScalars()` and the `{ ...updated, modules }` merge
      // below all have different shapes (only `existing` carries `lesson`
      // and `attachments`) — `any` avoids fighting the type checker over an
      // intermediate value that gets reshaped at the `return` below anyway.
      ...
      // `updated` may still be the raw `existing` fetch (untouched when only
      // `modules` changed), whose `attachments` carry `asset.bytes` as a
      // BigInt — dropped here rather than serialized, since this endpoint
      // isn't the source of truth for attachments (that's `getById` /
      // the dedicated attachment routes) and a stale/empty list would be
      // actively misleading.
      const { attachments: _attachments, lesson: _lesson, ...rest } = updated;
      return rest as AssignmentWithModules;
    });
  }
  ...
  private assertOwnership(
    ...
    if (actor.role === 'STUDENT') {
      // Reached only from a mutating path — students are read-only here.
      throw new ForbiddenException({
        code: 'FORBIDDEN_ROLE',
        message: 'Students cannot manage assignments',
      });
    }
  ...
  /**
   * Gate a STUDENT read on an APPROVED enrollment in the assignment's group.
   *
   * Mirrors the rule `LessonAccessGuard` applies to lesson content: paying
   * and being approved is what unlocks a group, and homework is group
  ...
  private async assertEnrolledStudent(
    ...
    const enrollment = await this.prisma.enrollment.findUnique({
      where: {
        groupId_studentId: { groupId, studentId: actor.userId },
      },
      select: { status: true },
    });
    ...
    if (enrollment?.status !== 'APPROVED') {
      throw new ForbiddenException({
        ...
        message:
          'You must be APPROVED in this group to view its homework. ' +
          'Pay and wait for teacher approval.',
      });
    }
  }
}
