...
export class HomeworkController {
  ...
  /**
   * GET /homework/my-assignments
   *
   * Every published assignment across the caller's APPROVED enrollments,
   * each paired with their own submission. Backs the student `/homework`
   * page, which previously built the same list client-side by walking
   * enrollments → lessons → assignments → submission — roughly a hundred
   * requests for a student in a few groups.
   */
  @Roles('STUDENT')
  @Get('homework/my-assignments')
  @HttpCode(HttpStatus.OK)
  async listMyAssignments(@CurrentUser() user: JwtPayload) {
    return this.homeworkService.listMyAssignments(user.sub);
  }
}
