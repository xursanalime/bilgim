 ...
 * (Task 17.4 from .kiro/specs/bilgim-platform/tasks.md)
 *
  ...
  it('P9a: createForLesson succeeds iff every requested module has GroupModule.isEnabled = true', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        // to mix enabled + disabled in the group toggle map. Restricted to
        // the eight Language_Module_Type values because assignment creation
        // now rejects Legacy_Module_Type with MODULE_TYPE_NOT_SUPPORTED
        // (refocus task 5.1) before the group-enabled gate is evaluated.
        fc
          .uniqueArray(fc.constantFrom(...LANGUAGE_MODULE_TYPES), {
            minLength: 2,
            maxLength: 6,
          })
          .chain((catalogTypes) =>
            fc
              ...
              .map(([flags, requestedSubset, withUnknown]) => ({
                ...
                requested: withUnknown
                  ? [
                      ...requestedSubset,
                      // pick an off-catalog LANGUAGE type for the negative
                      // path — it must still be one of the eight supported
                      // types so the request reaches the group-enabled gate
                      // (MODULE_NOT_ENABLED_FOR_GROUP) instead of being
                      // rejected earlier as an unsupported legacy type.
                      LANGUAGE_MODULE_TYPES.find(
  ...
  it('Non-destructive: flipping a module OFF leaves existing AssignmentModule rows intact', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        // using each one and then flip them all off. Restricted to the
        // eight Language_Module_Type values — assignment creation rejects
        // Legacy_Module_Type (refocus task 5.1), so witnesses must use
        // supported types.
