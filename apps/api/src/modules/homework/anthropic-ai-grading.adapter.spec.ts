...
describe('AnthropicAiGradingAdapter — English-teaching feedback dimensions (Req 9.2)', () => {
  it.each(['WRITING', 'GRAMMAR'] as const)(
    'attaches grammar/vocabulary/coherence dimensions for %s modules',
    async (moduleType) => {
      const gateway = buildGatewayStub();
      gateway.precheckWritingSubmission.mockResolvedValue({
        score: 0.8,
      ...
      const result = await adapter.gradeModule(
        buildWritingInput({
          ...
          answer: { text: 'The learner produced a paragraph of English.' },
        }),
      );

      expect(result.dimensions).toBeDefined();
      expect(result.dimensions!.grammar).toBeDefined();
      expect(result.dimensions!.vocabulary).toBeDefined();
      expect(result.dimensions!.coherence).toBeDefined();
      expect(result.dimensions!.grammar.score).toBeCloseTo(0.8);
      for (const skill of ['grammar', 'vocabulary', 'coherence'] as const) {
        expect(result.dimensions![skill].comment.length).toBeGreaterThan(0);
      }
    },
  );

  it('does NOT attach dimensions for READING modules', async () => {
    const gateway = buildGatewayStub();
    gateway.precheckWritingSubmission.mockResolvedValue({
      score: 0.6,
      summary: 'ok',
    ...
    const result = await adapter.gradeModule(
      buildWritingInput({ type: 'READING' as any }),
    );

    expect(result.dimensions).toBeUndefined();
  });

  it('keeps the dimension shape for an empty WRITING answer (no gateway call)', async () => {
    const gateway = buildGatewayStub();
    const adapter = new AnthropicAiGradingAdapter(gateway);
    ...
    expect(gateway.precheckWritingSubmission).not.toHaveBeenCalled();
    expect(result.dimensions).toBeDefined();
    expect(result.dimensions!.grammar.score).toBe(0);
    expect(result.dimensions!.grammar.comment).toMatch(/no answer/i);
  });
});
