 ...
 *
 * Notes:
 *   - Modules are optional — a purely descriptive assignment (e.g. "read
 *     chapter 3") with no interactive module is a valid assignment, but
 *     it must have *something* telling the student what to do: the
 *     cross-field refine below rejects title-only assignments that have
 *     neither a module nor a description.
...
export const CreateAssignmentSchema = z
  .object({
    ...
    modules: z.array(AssignmentModuleInputSchema).max(20),
  })
  .refine(
    (v) => v.modules.length > 0 || (v.description ?? '').trim().length > 0,
    {
      message:
        'Provide at least one module or a description explaining what the student should do',
      path: ['description'],
    },
  );
...
export const UpdateAssignmentSchema = z
  .object({
    ...
    modules: z.array(AssignmentModuleInputSchema).max(20).optional(),
  })
  .refine(
