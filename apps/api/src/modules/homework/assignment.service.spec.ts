...
describe('AssignmentService', () => {
  ...
  // STUDENT reads
  //
  // `/homework/[assignmentId]` and the assignment list on a lesson page
  // are student surfaces. They used to 403 for every student because
  // AssignmentsController carried a class-level @Roles('TEACHER','ADMIN'),
  // so the student homework pages could never load. Students are now
  // authorized by an APPROVED enrollment and only see published rows.
  // ==================================================================

  describe('STUDENT reads', () => {
    const STUDENT_ID = '99999999-9999-9999-9999-999999999999';
    const asStudent = { userId: STUDENT_ID, role: 'STUDENT' as const };

    function mockEnrollment(status: string | null): void {
      prisma.enrollment = {
        findUnique: jest
          .fn()
    ...
    describe('getById', () => {
      it('returns a published assignment to an APPROVED student', async () => {
        mockEnrollment('APPROVED');
        assignmentRepository.findByIdWithOwnership.mockResolvedValue(
          buildOwnership({ isPublished: true }),
        );

        const result = await service.getById(asStudent, ASSIGNMENT_ID);
      ...
      it('403s a student with no enrollment in the group', async () => {
        mockEnrollment(null);
        assignmentRepository.findByIdWithOwnership.mockResolvedValue(
          buildOwnership({ isPublished: true }),
        );
        ...
        await expect(
          service.getById(asStudent, ASSIGNMENT_ID),
        ).rejects.toBeInstanceOf(ForbiddenException);
      });

      it('403s a student whose enrollment is still PENDING_APPROVAL', async () => {
        mockEnrollment('PENDING_APPROVAL');
        assignmentRepository.findByIdWithOwnership.mockResolvedValue(
          buildOwnership({ isPublished: true }),
        );
      ...
      it('404s an unpublished draft rather than leaking its existence', async () => {
        mockEnrollment('APPROVED');
        assignmentRepository.findByIdWithOwnership.mockResolvedValue(
          buildOwnership({ isPublished: false }),
        );
    ...
    describe('listForLesson', () => {
      it('returns only published assignments', async () => {
        ...
        assignmentRepository.listByLesson.mockResolvedValue([
          buildAssignmentRow({ id: 'published-1', isPublished: true } as any),
          buildAssignmentRow({ id: 'draft-1', isPublished: false } as any),
        ]);

        const result = await service.listForLesson(asStudent, LESSON_ID);

        expect(result).toHaveLength(1);
        expect(result[0]!.id).toBe('published-1');
      });

      it('403s a student not enrolled in the lesson group', async () => {
        mockEnrollment(null);
        assignmentRepository.findLessonOwnership.mockResolvedValue({
          groupId: GROUP_ID,
          teacherId: TEACHER_ID,
        ...
        await expect(
          service.listForLesson(asStudent, LESSON_ID),
        ).rejects.toBeInstanceOf(ForbiddenException);
      });
    });

    it('never lets a STUDENT reach a mutating path', async () => {
      assignmentRepository.findByIdWithOwnership.mockResolvedValue(
        buildOwnership({ isPublished: true }),
      );

      await expect(
        service.publish(asStudent as any, ASSIGNMENT_ID),
      ).rejects.toBeInstanceOf(ForbiddenException);
    });
  });
});
