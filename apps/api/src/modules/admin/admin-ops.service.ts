...
/**
 * Consolidated KPI counters powering the admin dashboard stat cards
 * (`GET /admin/stats`). Mirrors the web `AdminDashboardStats` contract in
 * `apps/web/lib/api/admin.ts`. Every field is required and defaults to `0`
 * so the UI can render deterministic cards.
 */
...
/**
 * A single normalized entry in the admin recent-activity feed
 * (`GET /admin/activity`). Mirrors the web `AdminActivityItem` contract.
 */
...
export class AdminOpsService {
  ...
  // ==================================================================
  // Dashboard KPIs (GET /admin/stats)
  // ==================================================================

  /**
   * Consolidated KPI counters for the admin dashboard stat cards
   * (`GET /admin/stats`, web `AdminDashboardStats`).
   *
   * Each metric is a single indexed aggregate; running them in parallel
   * keeps the endpoint cheap. `revenueUzs` sums PAID invoice amounts over
   * the trailing 30 days. Missing aggregates default to `0`.
   */
  async getDashboardStats(): Promise<AdminDashboardStats> {
    ...
    const [
      ...
      this.prisma.invoice.aggregate({
        _sum: { amountUzs: true },
        where: { status: 'PAID', paidAt: { gte: last30d } },
      }),
    ...
    return {
      ...
      totalStudents,
      activeSubscriptions,
      revenueUzs: revenueAgg._sum.amountUzs ?? 0,
      liveSessions,
    };
  }

  // ==================================================================
  // Recent activity feed (GET /admin/activity)
  // ==================================================================

  /**
   * Normalized reverse-chronological activity feed for the admin overview
   * (`GET /admin/activity`, web `AdminActivityItem`).
   *
   * Unifies notable platform events from several sources — new user
   * signups, paid invoices, started live sessions and admin audit-log
   * entries — into a single normalized shape. Each source is queried for
   * its newest `limit` rows, merged, re-sorted by timestamp and trimmed,
   * so the feed stays accurate without a dedicated event table.
   */
  async getRecentActivity(limit = 8): Promise<{ items: AdminActivityItem[] }> {
    const take = Math.max(1, Math.min(limit, 50));

    const [users, paidInvoices, liveSessions, auditLogs] = await Promise.all([
      this.prisma.user.findMany({
        where: { status: { not: 'DELETED' } },
        orderBy: { createdAt: 'desc' },
        take,
      ...
      this.prisma.invoice.findMany({
        ...
        orderBy: { paidAt: 'desc' },
        take,
        select: { id: true, amountUzs: true, kind: true, paidAt: true },
      }),
    ...
    for (const u of users) {
      ...
      items.push({
        ...
        description:
          u.role === 'TEACHER'
            ? `${who} (o'qituvchi) ro'yxatdan o'tdi`
            : u.role === 'ADMIN'
              ? `${who} (admin) qo'shildi`
              : `${who} ro'yxatdan o'tdi`,
    ...
    for (const inv of paidInvoices) {
      items.push({
        ...
        description: `${numberFormatter.format(inv.amountUzs)} so'm${
          inv.kind === 'TEACHER_SUBSCRIPTION' ? " (obuna)" : ' (kurs)'
        }`,
        createdAt: (inv.paidAt as Date).toISOString(),
      });
    }
    ...
    for (const s of liveSessions) {
      items.push({
        id: `live:${s.id}`,
        type: 'live',
        title: s.status === 'LIVE' ? 'Jonli dars boshlandi' : 'Jonli dars',
        description: s.lesson?.title ?? null,
        createdAt: (s.startedAt as Date).toISOString(),
      });
    }

    for (const log of auditLogs) {
      const who = log.actor?.fullName || log.actor?.email || 'Tizim';
      items.push({
        id: `audit:${log.id}`,
        type: 'moderation',
        title: 'Admin amali',
        description: `${who}: ${log.action}${log.target ? ` → ${log.target}` : ''}`,
