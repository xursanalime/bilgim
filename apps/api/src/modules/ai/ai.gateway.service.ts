...
export class AiGatewayService implements AiGateway {
  ...
  async classifySpecialty(
    input: SpecialtyClassifyInput,
  ): Promise<SpecialtyClassifyOutput> {
    const dispatch = await this.dispatchIntent({
      userId: input.userId,
      rateLimitSubject: `user:${input.userId}`,
      intent: 'SPECIALTY_CLASSIFY',
      templateKey: PROMPT_TEMPLATE_KEYS.SPECIALTY_CLASSIFY,
      vars: {
        knownSpecialties: input.knownSpecialties.join(', '),
        answersJson: JSON.stringify(input.answersJson),
      },
      jsonMode: true,
      refType: 'specialty_classify',
      refId: input.userId,
    });

    const parsed = parseJsonStrict<SpecialtyClassifyOutput>(
      dispatch.response.text,
      {
        specialtySlug: input.knownSpecialties[0] ?? 'unknown',
        confidence: 0,
      },
    );
    await this.recordAudit({
    ...
    return {
      specialtySlug: parsed.specialtySlug ?? input.knownSpecialties[0] ?? 'unknown',
