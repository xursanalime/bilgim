...
const DEFAULT_TEMPLATES: Record<string, ResolvedPromptTemplate> = {
  ...
  [PROMPT_TEMPLATE_KEYS.SPECIALTY_CLASSIFY]: {
    key: PROMPT_TEMPLATE_KEYS.SPECIALTY_CLASSIFY,
    intent: 'SPECIALTY_CLASSIFY',
    systemText:
      'You classify student onboarding answers into one of the known specialty slugs. Reply with strict JSON: ' +
      '{ "specialtySlug": string, "confidence": number (0..1), "reasoning"?: string }.',
    userTemplate:
      'Known specialties: {{knownSpecialties}}\nAnswers: {{answersJson}}',
    modelName: 'claude-3-5-haiku-latest',
    maxTokens: 300,
  },
