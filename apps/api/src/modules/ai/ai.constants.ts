...
/**
 * AI intents retired by the English-only refocus (Req 8.5). The
 * `SPECIALTY_CLASSIFY` value still exists in the `AiIntent` Prisma enum
 * so historical `AiCall` audit rows stay readable (non-destructive
 * refocus), but the gateway rejects any attempt to dispatch it and the
 * prompt-template loader refuses to resolve its template key. Keeping
 * the list here gives a single auditable source of truth for "what was
 * removed and must never come back".
 */
export const REMOVED_AI_INTENTS = ['SPECIALTY_CLASSIFY'] as const;
export type RemovedAiIntent = (typeof REMOVED_AI_INTENTS)[number];

/**
 * Prompt-template keys retired alongside {@link REMOVED_AI_INTENTS}.
 * The loader throws `AiTemplateNotFoundError` for these regardless of
 * what is (or isn't) seeded in `AiPromptTemplate`.
 */
export const REMOVED_PROMPT_TEMPLATE_KEYS = ['specialty.classify.v1'] as const;
...
> = {
  ...
  // Gemini — Google AI public pricing as of the spec freeze, rounded up.
