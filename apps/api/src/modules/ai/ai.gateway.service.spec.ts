 ...
 *   - JSON-mode intents (translateWord, detectAiText)
 *     parse the structured response and survive malformed JSON.
 *   - SPECIALTY_CLASSIFY removal: the gateway exposes no
 *     classifySpecialty surface and the retired intent/template is
 *     rejected (Req 8.5).
 */
...
// ===================================================================
// English-teaching tutor intent set (Req 8.1)
// ===================================================================

describe('AiGatewayService — English tutor intent set (Req 8.1)', () => {
  const cases: Array<{
  ...
  }> = [
    {
      intent: 'EXPLAIN',
      templateKey: PROMPT_TEMPLATE_KEYS.TUTOR_EXPLAIN,
      auditIntent: 'TUTOR_EXPLAIN',
    },
    {
      intent: 'TRANSLATE',
      templateKey: PROMPT_TEMPLATE_KEYS.TUTOR_TRANSLATE,
      auditIntent: 'TUTOR_TRANSLATE',
    },
    {
      intent: 'EXAMPLE',
      templateKey: PROMPT_TEMPLATE_KEYS.TUTOR_EXAMPLE,
      auditIntent: 'TUTOR_EXAMPLE',
    },
  ];

  it.each(cases)(
    'dispatches the %s intent against its English-learning template and audits it',
    async ({ intent, templateKey, auditIntent }) => {
      const primary = new FakeAdapter({ providerName: 'fake-primary' });
      const { service, audit } = buildGateway({ primary });
      ...
      const result = await service.tutor({
        userId: STUDENT_ID,
        intent,
        question: 'Help me understand the present perfect tense.',
        locale: 'en',
      });
  ...
  it('exposes exactly the three English-teaching tutor methods', () => {
    const { service } = buildGateway();
    expect(typeof service.tutorExplain).toBe('function');
    expect(typeof service.tutorTranslate).toBe('function');
    expect(typeof service.tutorExample).toBe('function');
  });
});

// ===================================================================
// SPECIALTY_CLASSIFY removal (Req 8.5)
// ===================================================================

describe('AiGatewayService — SPECIALTY_CLASSIFY removed (Req 8.5)', () => {
  it('no longer exposes a classifySpecialty method on the gateway', () => {
    const { service } = buildGateway();
    expect(
      (service as unknown as Record<string, unknown>)['classifySpecialty'],
    ).toBeUndefined();
  });

  it('drops the SPECIALTY_CLASSIFY prompt-template key from the active catalog', () => {
    expect(
      (PROMPT_TEMPLATE_KEYS as Record<string, string>)['SPECIALTY_CLASSIFY'],
    ).toBeUndefined();
    expect(Object.values(PROMPT_TEMPLATE_KEYS)).not.toContain(
      'specialty.classify.v1',
    );
  });

  it('records SPECIALTY_CLASSIFY in the retired-intent registry', () => {
    expect(REMOVED_AI_INTENTS).toContain('SPECIALTY_CLASSIFY');
    expect(REMOVED_PROMPT_TEMPLATE_KEYS).toContain('specialty.classify.v1');
  });

  it('rejects resolution of the retired specialty.classify template', async () => {
    const loader = buildLoader();
    await expect(loader.resolve('specialty.classify.v1')).rejects.toBeInstanceOf(
      AiTemplateNotFoundError,
    );
  });
});
