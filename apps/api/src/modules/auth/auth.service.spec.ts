...
function buildMockUser(overrides: Partial<any> = {}) {
  return {
    ...
    location: null,
    mfaRequired: false,
    // Set when an admin (or a security event) revokes every session for the
    // user; `null` is the normal state. Part of the User row, so the fixture
    // has to carry it for the mocked repository to typecheck.
...
describe('AuthService', () => {
  ...
  beforeEach(() => {
    ...
    mockTx = {
      ...
      // Session revocation stamps `User.sessionsRevokedAt` inside the same
      // transaction as the password write, so it must exist on the tx client
      // as well as the root client.
      user: {
        update: jest.fn().mockResolvedValue({}),
      },
      teacherProfile: {
    ...
    mockPrisma = {
      ...
      // Global session revocation (token-family compromise, password reset)
      // stamps `User.sessionsRevokedAt` rather than deleting rows, so every
      // token issued before that instant is rejected on next use.
      user: {
        update: jest.fn().mockResolvedValue({}),
      },
    };
  ...
  describe('login', () => {
    ...
    it.each(['SUSPENDED', 'DELETED'] as const)(
      'throws 401 ACCOUNT_NOT_VERIFIED for a %s account',
      async (status) => {
        usersRepository.findByEmail.mockResolvedValue(buildMockUser({ status }));
    ...
    it('auto-activates a PENDING_VERIFY account instead of rejecting it', async () => {
      // The email-verification gate is currently open: rather than blocking
      // login, the service runs the same provisioning verifyEmail would have
      // (trial subscription + specialty) so teacher features do not break on
      // a missing TeacherProfile.
      usersRepository.findByEmail.mockResolvedValue(
        buildMockUser({ status: 'PENDING_VERIFY' }),
      );
      usersRepository.updateStatus.mockResolvedValue(
        buildMockUser({ status: 'ACTIVE' }) as any,
      );
      // Session issuance re-reads the user by id after the password check.
      usersRepository.findById.mockResolvedValue(
        buildMockUser({ status: 'ACTIVE' }) as any,
      );
      // The fixture's passwordHash is a placeholder, not a real Argon2 digest.
      jest.spyOn(authService, 'verifyPassword').mockResolvedValue(true);

      await expect(authService.login(loginDto, meta)).resolves.toMatchObject({
