...
export class AuthService {
  ...
  /**
   * Activate a user (status → ACTIVE) and run the teacher-only
   * provisioning (trial subscription + English specialty) that normally
   * happens at email-verification time. Idempotent — safe to call again
   * for an already-active user (startTrial/assignEnglishSpecialty are
   * upserts). Shared by `verifyEmail` and by `login`'s auto-activation
   * for accounts that never clicked the verification link.
   */
  private async activateUser(
    userId: string,
    tx: Prisma.TransactionClient,
  ): Promise<void> {
    const user = await this.usersRepository.updateStatus(userId, 'ACTIVE', tx);
  ...
  async verifyEmail(dto: VerifyEmailDto): Promise<VerifyEmailResult> {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await this.activateUser(verification.userId, tx);
    });
  ...
  async login(
    ...
    // Email verification gate disabled for now — only block accounts
    // that are explicitly suspended or deleted. Unverified accounts are
    // auto-activated on first login, running the same provisioning
    // (trial subscription + specialty) that verifyEmail would have done,
    // so teacher-only features don't break on a missing TeacherProfile.
    ...
    if (user.status === 'PENDING_VERIFY') {
      await this.prisma.$transaction((tx) => this.activateUser(user.id, tx));
      user.status = 'ACTIVE';
    }
