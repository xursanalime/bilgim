...
describe('AuthHardeningService — unlock token flow', () => {
  ...
  it('lets the user log in again once the token has been consumed', async () => {
    ...
    // Locks are keyed by the account identifier the failures were recorded
    // against (the email), not the userId — passing the userId here silently
    // looked up a lock that never existed, so the assertion passed vacuously.
    await expect(
      service.assertNotLocked(account, ip),
    ).rejects.toMatchObject({
