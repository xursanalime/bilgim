...
export class EnrollmentRepository {
  ...
  /**
   * Revoke an APPROVED enrollment (teacher removes a student from a group).
   * Conditional `updateMany` mirrors the approve/reject lost-update guard —
   * only an APPROVED row transitions, so a double-click can't double-revoke.
   */
  async revokeEnrollment(
    ...
    const result = await client.enrollment.updateMany({
      where: { groupId, studentId, status: 'APPROVED' },
