...
export class EnrollmentService {
  ...
  /**
   * Remove an APPROVED student from a group the calling teacher owns.
   *
   * Atomic: in one transaction we flip Enrollment → REVOKED and remove the
   * student's GroupChatMember row (mirroring the symmetric add-on-approve
   * in `approveRequest`), then emit `enrollment.revoked` to the outbox for
   * audit/notification fan-out, same shape as `rejectRequest`.
   */
  async revokeEnrollment(
    teacherId: string,
    groupId: string,
    studentId: string,
  ): Promise<{ groupId: string; studentId: string }> {
    const group = await this.prisma.group.findUnique({
      where: { id: groupId },
      include: { course: true },
    });
    ...
    if (group.course.teacherId !== teacherId) {
      throw new ForbiddenException({
        code: 'NOT_OWNING_TEACHER',
        message: 'You can only manage students in your own groups',
      });
    }

    await this.prisma.$transaction(async (tx) => {
      const revoked = await this.enrollmentRepository.revokeEnrollment(
        groupId,
        studentId,
        tx,
      );
      if (!revoked) {
        throw new NotFoundException({
          ...
          message: 'This student has no active enrollment in this group',
        });
      }

      await this.groupChatRepository.removeMember(groupId, studentId, tx);

      await this.outboxService.create(tx, {
        topic: 'enrollment.revoked',
        payload: { groupId, studentId, teacherId },
        idempotencyKey: `enrollment.revoked:${groupId}:${studentId}:${Date.now()}`,
      });
    });

    this.logger.log(
      `Revoked enrollment (group=${groupId} student=${studentId}) by teacher ${teacherId}`,
    );
