...
export class EnrollmentService {
  ...
  /**
   * POST /enrollment/invite/:token/join — Student uses invite link to
   * create an enrollment request. Atomically increments usesCount so the
   * usage cap is enforced correctly.
   *
   * BUG FIX: Previously usesCount was never incremented, so invite links
   * with a usesLimit would never expire.
   */
  async joinViaInvite(
    studentId: string,
    token: string,
    ...
    if (!group || group.status !== 'OPEN') {
      throw new ConflictException({
        code: 'GROUP_CLOSED',
        message: 'This group is not accepting new enrollment requests',
      });
    }
    ...
    const existingRequest = await this.prisma.enrollmentRequest.findFirst({
      where: { groupId: group.id, studentId, status: 'PENDING_APPROVAL' },
    });
    if (existingRequest) {
      throw new ConflictException({
        code: 'REQUEST_ALREADY_PENDING',
        message: 'You already have a pending enrollment request for this group',
      });
    }

    const request = await this.prisma.$transaction(async (tx) => {
      await this.enrollmentRepository.ensureStudentProfile(studentId, 'uz', tx);

      // BUG FIX: increment usesCount so usage cap is enforced
      await this.inviteLinkRepository.incrementUsesCount(invite.id, tx);

      const created = await this.enrollmentRepository.upsertPendingRequest(
        { groupId: group.id, studentId, message: null },
        tx,
      );
  ...
  /** Teacher: list active invite links for a group they own. */
  async listInviteLinks(teacherId: string, groupId: string): Promise<CreatedInvite[]> {
    const group = await this.prisma.group.findUnique({
      where: { id: groupId },
      include: { course: true },
    });
    if (!group || group.course.teacherId !== teacherId) {
      throw new ForbiddenException({ code: 'NOT_OWNING_TEACHER', message: 'Access denied' });
    }
    const links = await this.inviteLinkRepository.listActiveByGroup(groupId);
    return links.map((l) => this.toCreatedInvite(l));
  }

  /** Teacher: deactivate (revoke) an invite link they own. */
  async revokeInviteLink(teacherId: string, inviteLinkId: string): Promise<void> {
    const link = await this.inviteLinkRepository.findById(inviteLinkId);
    if (!link) {
      throw new NotFoundException({ code: 'INVITE_NOT_FOUND', message: 'Invite link not found' });
    }
    const group = await this.prisma.group.findUnique({
    ...
    await this.inviteLinkRepository.deactivate(inviteLinkId);
  }

  /** Student: cancel their own PENDING_APPROVAL request. */
  async cancelRequest(studentId: string, requestId: string): Promise<void> {
    const request = await this.prisma.enrollmentRequest.findUnique({
      where: { id: requestId },
    });
    if (!request || request.studentId !== studentId) {
      throw new NotFoundException({ code: 'ENROLLMENT_REQUEST_NOT_FOUND', message: 'Request not found' });
    }
    ...
    await this.prisma.enrollmentRequest.delete({ where: { id: requestId } });
  }
  ...
  async createRequest(
    ...
    // BUG FIX: guruh OPEN holatida bo'lishi shart
    ...
    // BUG FIX: allaqachon PENDING_APPROVAL so'rov mavjud bo'lsa 409 qaytarish
    const existingRequest = await this.prisma.enrollmentRequest.findFirst({
      where: { groupId: dto.groupId, studentId, status: 'PENDING_APPROVAL' },
    });
    if (existingRequest) {
      throw new ConflictException({
        code: 'REQUEST_ALREADY_PENDING',
