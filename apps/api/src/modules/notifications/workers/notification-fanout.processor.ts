...
export class NotificationFanoutProcessor extends WorkerHost {
  ...
  private async resolveRecipients(
    ...
    switch (topic) {
      ...
      case 'live.started':
        return this.recipientsForLiveStarted(payload);
      case 'live.reminder':
   ...
   */
  /**
   * A live class just went on air — tell every approved student now, while
   * they can still join. Without this the topic fell through to the default
   * branch and students were never told a class had started (Req 9.3).
   */
  private async recipientsForLiveStarted(
    payload: Record<string, unknown>,
  ): Promise<Array<{ userId: string; template: ResolvedTemplate }>> {
    const lessonId = payload.lessonId as string | undefined;
    const groupId = payload.groupId as string | undefined;
    if (!lessonId || !groupId) {
      this.logger.warn(
        `live.started payload missing lessonId/groupId: ${JSON.stringify(payload)}`,
      );
      return [];
    }
    ...
    return enrollments.map((e) => ({
      userId: e.studentId,
      template: {
        kind: 'LIVE_STARTED' as NotificationKind,
        title: 'Jonli dars boshlandi',
        body: lesson?.title
          ? `"${lesson.title}" jonli darsi hozir boshlandi. Qo'shiling!`
          : 'Jonli darsingiz hozir boshlandi. Qo‘shiling!',
        data: {
          lessonId,
          groupId,
  ...
  private buildIdempotencyKey(
    ...
    switch (topic) {
      ...
      case 'live.started':
        // Keyed by sessionId: restarting a lesson is a new session and
        // legitimately notifies again, but a redelivered job must not.
        return `live.started:${payload.sessionId ?? payload.lessonId}:${userId}`;
      case 'live.reminder':
