    ...
    await fc.assert(
      fc.asyncProperty(worldArb, async ({ numGroups, ops }) => {
        ...
        for (const e of world.state.outboxEvents) {
          // Uniqueness is per schedule event, not per group: two schedules in
          // the same group each version independently, so (group, version)
          ...
          // Idempotency-key shape is part of the public contract. Keyed by
          // scheduleId: a group can own several schedule events, so keying on
          // the group would collapse two distinct events sharing a version.
