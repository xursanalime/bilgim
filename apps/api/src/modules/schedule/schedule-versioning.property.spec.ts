 ...
 * (Task 14.4 from .kiro/specs/bilgim-platform/tasks.md)
 *
  ...
  it('per-group outbox versions are strictly monotonic 1..N between consecutive CREATEs (sequential ops)', async () => {
    await fc.assert(
      fc.asyncProperty(worldArb, async ({ numGroups, ops }) => {
        ...
        // I2 — global uniqueness of (scheduleId, version) across the whole
        ...
        for (const e of world.state.outboxEvents) {
          const key = `${e.payload.scheduleId}:${e.payload.version}`;
          expect(seenPairs.has(key)).toBe(false);
          seenPairs.add(key);
          expect(e.topic).toBe('schedule.changed');
          // Idempotency-key shape is part of the public contract. The
          // key is scoped to the schedule row (scheduleId) + version so
          // a create→delete→create cycle (version resets to 1 on the
          // fresh row) never collides with the prior row's v1 event.
          expect(e.idempotencyKey).toBe(
            `schedule.changed:${e.payload.scheduleId}:v${e.payload.version}`,
          );
        }
