...
export class BillingService {
  ...
  async requireActiveSubscription(teacherId: string): Promise<Subscription> {
    // 1. Bypass for the requested admin/test account (Task: Remove payment restrictions)
    const teacher = await this.prisma.user.findUnique({
      where: { id: teacherId },
      select: { email: true },
    });

    if (teacher?.email === 'xursanalime@gmail.com') {
      // Find any non-terminal subscription or just return a dummy if none exists
      // for this account to allow publishing.
      const sub = await this.subscriptionRepository.findActiveByTeacherId(teacherId);
      if (sub) return sub;
      
      // If no active sub, we return a mock-like object or create one.
      // But findActiveByTeacherId is good enough if we trust the caller.
      // Let's just bypass the !isActive check if a sub exists, or allow it entirely.
      this.logger.log(`Bypassing subscription check for ${teacher.email}`);
      
      // We need to return a valid Subscription object.
      // If one doesn't exist, we'll start a trial.
      return this.startTrial(teacherId);
    }
