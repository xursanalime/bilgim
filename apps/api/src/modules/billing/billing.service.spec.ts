...
describe('BillingService', () => {
  ...
  beforeEach(() => {
    ...
    prisma = {
      ...
      // Demo-mode gate (`billing.requireSubscription`). Defaults to "no row",
      // i.e. the paywall is enforced.
      systemSetting: {
  ...
  describe('requireActiveSubscription', () => {
    ...
    it('reports SUBSCRIPTION_MISSING when no subscription row exists', async () => {
      // Distinct from EXPIRED: the account was activated outside the normal
      // verify-email path and never got a trial provisioned, so "renew your
      // plan" would be misleading advice.
      subscriptionRepository.findActiveByTeacherId.mockResolvedValue(null);

      await expect(
        service.requireActiveSubscription(TEACHER_ID),
      ).rejects.toMatchObject({
        response: expect.objectContaining({ code: 'SUBSCRIPTION_MISSING' }),
      });
    });
    ...
    // ----------------------------------------------------------------
    // Demo mode (SystemSetting `billing.requireSubscription`)
    //
    // Replaces two hardcoded `email === 'xursanalime@gmail.com'` bypasses
    // that previously sat in billing.service.ts and catalog.service.ts.
    // The gate must fail closed: only an explicit `false` opens it.
    // ----------------------------------------------------------------

    describe('demo mode', () => {
      function setGate(value: unknown): void {
        prisma.systemSetting.findUnique.mockResolvedValue(
          value === undefined ? null : { key: 'billing.requireSubscription', value },
        );
      }

      it('lets an expired teacher publish when the gate is turned off', async () => {
      ...
      it('accepts the { enabled: false } object shape', async () => {
        const sub = buildSubscription({ status: 'PAST_DUE' });
        subscriptionRepository.findActiveByTeacherId.mockResolvedValue(sub);
      ...
      it('starts a real trial when a gated-off teacher has no subscription', async () => {
        subscriptionRepository.findActiveByTeacherId.mockResolvedValue(null);
        const trial = buildSubscription({ status: 'TRIAL' });
        subscriptionRepository.createTrial.mockResolvedValue(trial);
        setGate(false);

        await expect(
          service.requireActiveSubscription(TEACHER_ID),
        ).resolves.toBeTruthy();
        expect(subscriptionRepository.createTrial).toHaveBeenCalled();
      });

      it.each([
        ['a missing setting row', undefined],
        ['an explicit true', true],
        ['{ enabled: true }', { enabled: true }],
        ['a malformed value', 'yes'],
      ])('still enforces on %s', async (_label, value) => {
        subscriptionRepository.findActiveByTeacherId.mockResolvedValue(
          buildSubscription({ status: 'PAST_DUE' }),
        );
        setGate(value);
      ...
      it('enforces when the settings lookup itself fails', async () => {
        subscriptionRepository.findActiveByTeacherId.mockResolvedValue(
          buildSubscription({ status: 'PAST_DUE' }),
        );
        prisma.systemSetting.findUnique.mockRejectedValue(
