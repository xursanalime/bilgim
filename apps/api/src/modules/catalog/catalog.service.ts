...
export class CatalogService {
   ...
   * the eight `GroupModule` rows from the single global English module
   * catalog (English-Only Platform Refocus, Req 6.4, 6.5).
   *
   * The seeding source is the platform-wide
   * `GLOBAL_ENGLISH_MODULE_CATALOG` — the eight Language_Module_Type
   * skills — **not** the teacher's `Specialty` catalog. Groups no longer
   * depend on a Specialty being assigned (Req 2.2); legacy `Specialty`
   * rows are retained read-only (Req 16.5) but never consulted here.
   *
   * Idempotency (Req 6.4): seeding goes through
   * `GroupModuleRepository.seedForGroup`, which `createMany`s with
   * `skipDuplicates` against the `(groupId, moduleType)` primary key, so
   * re-seeding a group that already has its modules never duplicates rows.
   *
   * Errors:
  ...
  async createGroup(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // Seed exactly the eight Language_Module_Type skills from the global
      // English catalog (Req 6.4). `seedForGroup` is idempotent via
      // `skipDuplicates`, so this is safe even if a module row already
      // exists for the group.
      const modules = await this.groupModuleRepository.seedForGroup(
        {
          groupId: group.id,
          modules: GLOBAL_ENGLISH_MODULE_CATALOG.map((m) => ({
      ...
      this.logger.log(
        `Group ${group.id} created in course ${courseId} (teacher=${teacherId}); seeded ${modules.length} global English module toggles`,
      );

      return this.serializeGroup(Object.assign(group, { modules }));
    });
  }
  ...
  async listGroupsForCourse(
    ...
    await this.assertCourseOwnership(teacherId, courseId);
    const groups = await this.groupRepository.listByCourse(courseId);
  ...
  async getGroupForUser(
    ...
    if (role === 'ADMIN') {
      ...
      return this.serializeGroup(plainGroup as Group);
    }
  ...
  async getGroupForTeacher(teacherId: string, id: string): Promise<Group> {
    return this.serializeGroup(await this.assertGroupOwnership(teacherId, id));
  }
  ...
  /**
   * Update a Group's server-authoritative protection settings — currently
   * just the `downloadAllowed` Download_Permission toggle (Content
   * Protection Req 4.5). Backs `PATCH /catalog/groups/:id/settings`.
   *
   * Authorization (Req 4.4 — server-enforced, never trusts a client flag):
   *   - ADMIN may toggle any group.
   *   - A TEACHER may toggle only a group whose parent course they own;
   *     `assertGroupOwnership` raises 404 GROUP_NOT_FOUND / 403
   *     NOT_OWNING_TEACHER, matching the rest of the catalog surface.
   *
   * The new value is persisted and the refreshed row is read back from the
   * database before being serialized, so the response reflects the
   * authoritative stored state rather than the request body.
   */
  async updateGroupSettings(
    ...
    if (actorRole === 'ADMIN') {
      // ADMIN bypass — confirm existence, then write unscoped.
      ...
      // Owning-teacher path — 404/403 before any write.
      await this.assertGroupOwnership(actorUserId, id);
      const result = await this.groupRepository.updateForTeacher(
        id,
        actorUserId,
        { downloadAllowed: dto.downloadAllowed },
      );
      if (result.count === 0) {
        // Unreachable after the assert above, but keeps the contract honest
    ...
    const refreshed = await this.groupRepository.findById(id);
    if (!refreshed) {
      throw new NotFoundException({
        code: 'GROUP_NOT_FOUND',
        message: `Group ${id} not found`,
    ...
    const { course: _course, ...plainGroup } = refreshed;
    this.logger.log(
      `Group ${id} downloadAllowed set to ${dto.downloadAllowed} by ${actorRole} ${actorUserId}`,
    );
    return this.serializeGroup(plainGroup as Group);
  }
  ...
  async listLessonsForTeacher(
    ...
    const lessons = await this.lessonRepository.listByGroup(groupId);
    // Surface the parent Group's server-authoritative Download_Permission on
    // each lesson DTO for display (Content Protection Req 4.5).
    const downloadAllowed = (group as Group).downloadAllowed ?? false;
    return lessons.map((l) =>
      this.serializeLessonWithDownloadPermission(l, downloadAllowed),
    );
  }

  async listReadyLessonsForGroup(groupId: string): Promise<Lesson[]> {
    const group = await this.groupRepository.findById(groupId);
    const downloadAllowed = group?.downloadAllowed ?? false;
    const lessons = await this.lessonRepository.listByGroupAndStatus(
      groupId,
  ...
  /**
   * Normalize a Group row into a response DTO with a guaranteed
   * server-authoritative `downloadAllowed` boolean (Content Protection
   * Req 4.5). The value is whatever the database holds; a missing/undefined
   * value coerces to the schema default (`false`) so the client never has to
   * infer it and can never override it. Every other field on the row is
   * preserved as-is (including the seeded `modules` on a freshly created
   * group).
   */
  private serializeGroup<T extends { downloadAllowed?: boolean | null }>(
    group: T,
  ): T & { downloadAllowed: boolean } {
    return { ...group, downloadAllowed: group.downloadAllowed ?? false };
  }

  /**
   * Surface the parent Group's server-authoritative `downloadAllowed` flag
   * on a lesson response DTO (Content Protection Req 4.5). Lessons inherit
   * the Download_Permission of their Group; the flag is attached for display
   * only — the protected-file streaming path re-checks it server-side on
   * every request (Req 4.4).
   */
  private serializeLessonWithDownloadPermission<T>(
    lesson: T,
    downloadAllowed: boolean,
  ): T & { downloadAllowed: boolean } {
    return { ...lesson, downloadAllowed };
  }
  ...
  /**
   * Update an attachment's properties (role, caption, position) owned by the calling teacher.
   */
  async updateAttachment(
    teacherId: string,
    attachmentId: string,
    dto: { role?: string; caption?: string | null; position?: number },
  ) {
    const attachment = await this.prisma.attachment.findUnique({
      where: { id: attachmentId },
      include: {
