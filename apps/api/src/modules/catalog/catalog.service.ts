...
export class CatalogService {
  ...
  async createGroup(
    ...
    // A group seeds its homework-module toggles from the teacher's specialty
    // catalog, so onboarding has to have assigned one first.
    const profile = await this.teacherProfileRepository.findByUserId(teacherId);
    const specialtyId = profile?.specialtyId;
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // Auto-provision the group's chat room with the teacher as OWNER,
      // so "Guruh" chats always exist the moment a course cohort opens.
      await this.groupChatRepository.upsertRoomForGroup(group.id, tx);
      await this.groupChatRepository.addOwner(group.id, teacherId, tx);
  ...
  private async assertGroupOwnership(
    ...
    // Strip the joined `course` field — it exists only for the ownership
    // check and is not part of the public shape.
    const { course: _course, ...plainGroup } = group;
    return plainGroup;
  }
  ...
  /**
   * List attachments for an assignment the teacher owns (via
   * assignment → lesson → group → course.teacherId). Same shape as the
   * lesson variant — these are the instructional files (video/PDF/etc.)
   * a teacher attaches alongside the assignment description, distinct
   * from the student's own submitted work.
   */
  async listAttachmentsForAssignment(teacherId: string, assignmentId: string) {
    await this.assertAssignmentOwnership(teacherId, assignmentId);
    return this.getAssignmentAttachmentsUnchecked(assignmentId);
  }

  /**
   * Same query as `listAttachmentsForAssignment`, minus the teacher-
   * ownership check — for callers where access was already gated
   * upstream (e.g. the student-facing assignment/submit pages, which
   * admit any APPROVED-enrolled student, not just the owning teacher).
   */
  async getAssignmentAttachmentsUnchecked(assignmentId: string) {
    const rows = await this.prisma.attachment.findMany({
      where: { assignmentId },
      orderBy: [{ position: 'asc' }, { createdAt: 'asc' }],
      include: {
  ...
  /**
   * Link an UPLOADED MediaAsset to an Assignment via the Attachment
   * table. Mirrors `createAttachment` (the lesson variant) exactly,
   * just against the assignment ownership chain.
   */
  async createAttachmentForAssignment(
    teacherId: string,
    assignmentId: string,
    dto: {
      assetId: string;
    ...
    const position =
      dto.position ??
      (await this.prisma.attachment.count({ where: { assignmentId } }));
  ...
  private async assertAssignmentOwnership(
    ...
    if (assignment.lesson.group.course.teacherId !== teacherId) {
      throw new ForbiddenException({
        code: 'NOT_OWNING_TEACHER',
   ...
   *
   * An attachment hangs off exactly one parent (lesson XOR assignment —
   * enforced at the application layer, not the DB), so exactly one of
   * the two ownership chains below will be non-null.
   */
  async deleteAttachment(teacherId: string, attachmentId: string): Promise<void> {
    ...
    const ownerTeacherId =
      attachment.lesson?.group.course.teacherId ??
      attachment.assignment?.lesson.group.course.teacherId;
