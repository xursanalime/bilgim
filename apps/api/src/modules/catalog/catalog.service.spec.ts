...
describe('CatalogService', () => {
  ...
  beforeEach(() => {
    ...
    prisma = {
      ...
      user: {
        // createGroup looks up the teacher's email to apply the configured
        // account bypass. Default to a non-bypass email so these tests
        // exercise the normal onboarding-enforcement path.
        findUnique: jest
          .fn()
          .mockResolvedValue({ email: 'teacher@example.com' }),
      },
      specialty: {
        // Bypass fallback only — non-bypass accounts never reach it.
        findFirst: jest.fn().mockResolvedValue(null),
      },
      group: {
        // allocateUniqueJoinCode probes for an existing join code;
        // null means the freshly generated code is unique.
  ...
  // ==================================================================
  // createGroup + global English catalog seeding (Req 6.4, 6.5, 2.2)
  // ==================================================================

  describe('createGroup', () => {
    // The eight Language_Module_Type skills the global English catalog seeds
    // on every group creation (Req 6.4). Spelled out so the test fails loudly
    // if the global catalog ever drifts from these eight skills.
    ...
    /**
     * Echo the seeded modules back the way the real repository would: one
     * GroupModule row per requested skill, isEnabled = defaultEnabled,
     * sorted asc by moduleType.
     */
    function wireSeedEcho() {
    ...
    it('seeds exactly the eight global English skills on group creation (Req 6.4)', async () => {
      ...
      // Exactly one GroupModule per Language_Module_Type — eight rows.
      expect(result.modules).toHaveLength(8);
      const seededTypes = (
        groupModuleRepository.seedForGroup.mock.calls[0]![0] as any
      ).modules.map((m: any) => m.moduleType);
      expect(new Set(seededTypes)).toEqual(new Set(EXPECTED_SKILLS));

      // All eight are seeded enabled by default (Req 6.4 default state).
      const enabledTypes = result.modules
        .filter((m) => m.isEnabled)
        .map((m) => m.moduleType);
      expect(new Set(enabledTypes)).toEqual(new Set(EXPECTED_SKILLS));
    });

    it('does NOT consult the per-Specialty catalog (replaced by the global English catalog, Req 6.5)', async () => {
      setupSuccessfulOwnership();
      wireSeedEcho();

      await service.createGroup(TEACHER_ID, COURSE_ID, {
        name: 'g',
        priceUzs: 0,
      });

      // The specialty catalog is no longer the seeding source.
      expect(specialtyService.listActiveModules).not.toHaveBeenCalled();
    });

    it('does not require a specialty: seeds the eight skills even with no teacher profile (Req 2.2)', async () => {
      setupSuccessfulOwnership();
      wireSeedEcho();
      // No specialty on profile — group creation must still succeed.
    ...
    it('seeds idempotently — passes the seed through the skipDuplicates-backed repository (Req 6.4)', async () => {
      setupSuccessfulOwnership();
      wireSeedEcho();
      ...
      // The service delegates dedup to GroupModuleRepository.seedForGroup,
      // which createMany()s with skipDuplicates against the
      // (groupId, moduleType) primary key. Each skill appears exactly once
      // in the seed payload — no duplicate module types.
      const seeded = (
        groupModuleRepository.seedForGroup.mock.calls[0]![0] as any
      ).modules.map((m: any) => m.moduleType);
      expect(seeded).toHaveLength(8);
      expect(new Set(seeded).size).toBe(8);
    });
  ...
  describe('listGroupsForCourse', () => {
    ...
    it('returns groups for an owned course', async () => {
      ...
      // Each group surfaces a server-authoritative downloadAllowed (Req 4.5);
      // a row without the column coerces to the schema default `false`.
      expect(result).toEqual([
        { id: GROUP_ID, name: 'g', downloadAllowed: false },
      ]);
    });
  });
  ...
  describe('listLessonsForTeacher', () => {
    it('returns all lessons regardless of status for the owning teacher', async () => {
      ...
      // Each lesson DTO inherits the parent Group's server-authoritative
      // downloadAllowed for display (Req 4.5); the mock group has no column
      // set, so it coerces to the schema default `false`.
      expect(result).toEqual(
        lessons.map((l) => ({ ...l, downloadAllowed: false })),
      );
    });
  ...
  // ==================================================================
  // downloadAllowed surfacing + group-settings toggle
  // (Content Protection Req 4.5, 4.4)
  // ==================================================================

  describe('downloadAllowed surfacing on Group DTOs (Req 4.5)', () => {
    it('surfaces the server-authoritative downloadAllowed=true from the DB on getGroupForTeacher', async () => {
      groupRepository.findById.mockResolvedValue({
        id: GROUP_ID,
        courseId: COURSE_ID,
        name: 'g',
      ...
      // The joined course must never leak onto the DTO.
      expect((result as any).course).toBeUndefined();
    });

    it('defaults downloadAllowed to false when the column is absent/false (Req 4.5 default)', async () => {
      groupRepository.findById.mockResolvedValue({
        id: GROUP_ID,
        courseId: COURSE_ID,
        name: 'g',
    ...
    it('surfaces downloadAllowed on each lesson DTO from the parent group (Req 4.5)', async () => {
      groupRepository.findById.mockResolvedValue({
        id: GROUP_ID,
        courseId: COURSE_ID,
        downloadAllowed: true,
      ...
      lessonRepository.listByGroup.mockResolvedValue([
        { id: 'l1', status: 'READY' },
      ] as any);

      const result = await service.listLessonsForTeacher(TEACHER_ID, GROUP_ID);
      expect(result).toEqual([
        { id: 'l1', status: 'READY', downloadAllowed: true },
      ]);
    });
  });

  describe('updateGroupSettings (Req 4.5, 4.4)', () => {
    it('lets the owning teacher toggle downloadAllowed and returns the persisted value', async () => {
      // assertGroupOwnership read, then the post-update refetch.
      groupRepository.findById
        .mockResolvedValueOnce({
          id: GROUP_ID,
          courseId: COURSE_ID,
      ...
      // ADMIN-only unscoped path must not be used for a teacher.
      expect(groupRepository.updateById).not.toHaveBeenCalled();
      expect(result.downloadAllowed).toBe(true);
      expect((result as any).course).toBeUndefined();
    });

    it('rejects a non-owning teacher with 403 NOT_OWNING_TEACHER and never writes', async () => {
      groupRepository.findById.mockResolvedValue({
        id: GROUP_ID,
        courseId: COURSE_ID,
        course: { teacherId: OTHER_TEACHER_ID },
      ...
      await expect(
        service.updateGroupSettings(TEACHER_ID, 'TEACHER', GROUP_ID, {
          downloadAllowed: true,
        }),
      ).rejects.toMatchObject({ response: { code: 'NOT_OWNING_TEACHER' } });
      expect(groupRepository.updateForTeacher).not.toHaveBeenCalled();
      expect(groupRepository.updateById).not.toHaveBeenCalled();
    });

    it('throws 404 GROUP_NOT_FOUND when the group does not exist', async () => {
      groupRepository.findById.mockResolvedValue(null);
    ...
    it('lets an ADMIN toggle any group via the unscoped update path', async () => {
      groupRepository.findById
        .mockResolvedValueOnce({
          id: GROUP_ID,
          courseId: COURSE_ID,
      ...
      groupRepository.updateById.mockResolvedValue({ count: 1 });
      ...
      expect(groupRepository.updateById).toHaveBeenCalledWith(GROUP_ID, {
        downloadAllowed: true,
      });
      // ADMIN must not be routed through the teacher-scoped write.
      expect(groupRepository.updateForTeacher).not.toHaveBeenCalled();
      expect(result.downloadAllowed).toBe(true);
    });
  });
