...
describe('LessonsController', () => {
  ...
  beforeEach(() => {
    service = {
      ...
      // `GET /catalog/lessons/:id` enriches the guard-resolved lesson with
      // its attachments; without this the controller throws before it can
      // return anything.
      getLessonAttachmentsUnchecked: jest.fn().mockResolvedValue([]),
  ...
  it('GET /catalog/lessons/:id returns the LessonAccessGuard-resolved lesson with its attachments', async () => {
    const lesson = { id: 'l1', status: 'READY' };
    const attachments = [{ id: 'a1', mediaAssetId: 'm1' }];
    (service.getLessonAttachmentsUnchecked as jest.Mock).mockResolvedValue(
      attachments,
    );

    const result = await controller.getOne({ lesson } as any);

    // The guard already authorized the lesson, so the controller looks the
    // attachments up unchecked and merges them onto the resolved row.
    expect(service.getLessonAttachmentsUnchecked).toHaveBeenCalledWith('l1');
    expect(result).toEqual({ ...lesson, attachments });
  });
