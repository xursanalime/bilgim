...
export interface TeacherAnalyticsStudentRow {
  ...
  /**
   * `PAID` — latest Invoice for this (student, group) pair is PAID.
   * `PENDING` — an Invoice exists but never reached PAID (pending/canceled/refunded).
   * `NONE` — no Invoice at all (free group, or teacher approved without payment).
   */
...
export class TeacherAnalyticsService {
  ...
  async getStudentProgress(
    ...
    // Payment status per (student, group) pair — most recent Invoice wins.
    // No Invoice row at all means the student was approved without ever
    // going through checkout (free group, or a manual/direct-join approval).
    const invoices = await this.prisma.invoice.findMany({
      where: {
        groupId: { in: groupIds },
        studentId: { in: enrollments.map((e) => e.studentId) },
    ...
    for (const inv of invoices) {
      if (!inv.groupId || !inv.studentId) continue;
      const key = `${inv.studentId}::${inv.groupId}`;
      if (!latestInvoiceStatus.has(key)) {
        latestInvoiceStatus.set(key, inv.status);
      }
    }
    ...
    return enrollments
      .map((e): TeacherAnalyticsStudentRow => {
        ...
        const invoiceStatus = latestInvoiceStatus.get(key);
        const paymentStatus: TeacherAnalyticsStudentRow['paymentStatus'] =
          invoiceStatus === 'PAID' ? 'PAID' : invoiceStatus ? 'PENDING' : 'NONE';
        return {
