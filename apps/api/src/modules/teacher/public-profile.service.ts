...
export class TeacherPublicProfileService {
  ...
  async setStatus(
    ...
    try {
      ...
      // Concurrent request claimed `data.publicSlug` between the
      // `assertSlugClaimable` check above and this write — the unique
      // constraint on `TeacherProfile.publicSlug` is the source of truth,
      // translate its P2002 into the same 409 the check would give.
      if (
        data.publicSlug &&
        ...
        throw new ConflictException({
          code: 'SLUG_TAKEN',
          message: `"${data.publicSlug}" allaqachon band — boshqa nom tanlang.`,
        });
      }
      throw error;
    }
