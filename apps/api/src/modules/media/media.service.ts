...
export interface MediaAssetMetadataResponse {
  id: string;
  kind: MediaKind;
  fileName: string;
  sizeBytes: number;
...
/** One entry of `MediaService.getAssetsBatch`'s response array. */
export interface MediaAssetBatchResult {
  assetId: string;
  metadata: MediaAssetMetadataResponse | null;
  playback: MediaPlaybackResponse | null;
  /** Error code (e.g. `MEDIA_NOT_FOUND`, `MEDIA_ACCESS_DENIED`) when this asset couldn't be resolved. */
  error: string | null;
}
...
export class MediaService {
  ...
  async initiateUpload(
    ...
    this.assertSafeFileExtension(dto.fileName);
  ...
  /**
   * Resolve metadata + a signed playback URL for many assets in one
   * call (chat bubbles rendering a page of attachments — see
   * `BatchAssetUrlsSchema` for why this replaces one HTTP round-trip
   * per attachment).
   *
   * Runs the same `getAssetMetadata` + `getPlaybackUrls` access checks
   * per asset, but a failure on one id (not found, no access, still
   * uploading) is reported inline instead of failing the whole batch —
   * a chat page with 19 readable attachments and 1 stale/deleted one
   * should still render the 19.
   */
  async getAssetsBatch(
    actor: MediaAccessActor,
    assetIds: string[],
    expiresInSeconds: number = MEDIA_PLAYBACK_DEFAULT_TTL_SECONDS,
  ): Promise<MediaAssetBatchResult[]> {
    return Promise.all(
      assetIds.map(async (assetId): Promise<MediaAssetBatchResult> => {
        try {
          const [metadata, playback] = await Promise.all([
            this.getAssetMetadata(actor.sub, assetId),
            this.getPlaybackUrls(actor, assetId, expiresInSeconds),
          ]);
          return { assetId, metadata, playback, error: null };
        } catch (err) {
          const code =
            err instanceof NotFoundException ||
            err instanceof ConflictException ||
            err instanceof ForbiddenException
              ? ((err.getResponse() as { code?: string })?.code ?? 'MEDIA_ERROR')
              : 'MEDIA_ERROR';
          return { assetId, metadata: null, playback: null, error: code };
        }
      }),
    );
  }
  ...
  /**
   * Defense-in-depth backstop for `assertContentTypeAllowed`: the client
   * controls both the filename and the declared `contentType`, so a
   * mismatched pair (`invoice.pdf.exe` declared as `kind=OTHER` /
   * `text/plain`) would otherwise sail through on content-type alone.
   * Checked against every upload regardless of kind.
   */
  private assertSafeFileExtension(fileName: string): void {
    if (hasBlockedExtension(fileName)) {
      throw new BadRequestException({
        code: 'MEDIA_FILE_TYPE_BLOCKED',
