...
import { BatchAssetUrlsDto, BatchAssetUrlsSchema } from './dto';
import {
  MediaService,
...
...
  ...
  /**
   * `POST /media/assets/batch-urls` — metadata + a signed playback URL
   * for up to 50 assets in one request (Body: `{ assetIds[] }`).
   *
   * Replaces the per-attachment `GET /media/assets/:id` +
   * `GET /media/assets/:id/url` pair chat bubbles used to fire on
   * mount — a 20-attachment page of chat history went from 40
   * round-trips to 1. A `POST` (not `GET`) because `assetIds` can
   * exceed a comfortable query-string length and this is a read with
   * a body, mirroring how the rest of the API handles bulk lookups.
   *
   * Never throws per-asset: an inaccessible/missing/not-ready id shows
   * up as `{ assetId, metadata: null, playback: null, error: CODE }`
   * in its slot instead of failing the whole batch.
   */
  @Post('batch-urls')
  @HttpCode(HttpStatus.OK)
  async getAssetsBatch(
    @CurrentUser() user: JwtPayload,
    @Body(new ZodValidationPipe(BatchAssetUrlsSchema)) dto: BatchAssetUrlsDto,
    @Query(new ZodValidationPipe(PlaybackQuerySchema)) query: PlaybackQuery,
  ): Promise<MediaAssetBatchResult[]> {
    return this.mediaService.getAssetsBatch(
      { sub: user.sub, role: user.role },
      dto.assetIds,
      query.expiresIn ?? MEDIA_PLAYBACK_DEFAULT_TTL_SECONDS,
    );
