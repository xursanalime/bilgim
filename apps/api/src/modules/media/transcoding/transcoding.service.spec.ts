...
describe('TranscodingService', () => {
  ...
  beforeEach(() => {
    r2 = {
      ...
      // Drain any Readable body so the underlying file handle is opened
      // and read to completion BEFORE the service's `finally` cleanup
      // removes the temp dir — mirrors the real R2 client, which fully
      // consumes the upload stream before resolving.
  ...
  it('downloads the source, encodes every variant, writes master + uploads everything (Req 8.4, 8.5)', async () => {
    ...
    // Source fetched once (streamed to disk).
  ...
  it('cleans up the temp working directory after success', async () => {
    ...
    // No new bilgim-transcode-* directories left behind.
  ...
  it('cleans up the temp working directory after failure', async () => {
    ...
    const dirs = fs.readdirSync(os.tmpdir()).filter((n) =>
      n.includes(`bilgim-transcode-${ASSET_ID}`),
    );
    expect(dirs).toEqual([]);
  });
