...
export class VideoTranscodeProcessor extends WorkerHost {
  ...
  /**
   * Whether HLS transcoding is available in this deployment. Mirrors
   * `MediaService.isTranscodingAvailable`: disabled on a local single-box
   * media server (no ffmpeg worker) or when `MEDIA_TRANSCODING_ENABLED=false`.
   * Defaults to enabled when no config is injected (unit tests).
   */
  private isTranscodingAvailable(): boolean {
    if (!this.config) return true;
    if (this.config.get<string>('STORAGE_DRIVER') === 'local') return false;
  ...
  async process(job: Job<VideoTranscodeJob>): Promise<VideoTranscodeResult> {
    ...
    // Transcoding disabled (local single-box / no ffmpeg worker): never run
    // ffmpeg. Mark the video READY for progressive playback of the original
    // file so it is never stranded in TRANSCODING. This also drains any
    // stale jobs enqueued while transcoding was previously enabled.
    if (!this.isTranscodingAvailable()) {
      if (!asset.originalKey) {
        return { assetId, status: 'SKIPPED' };
      }
      if (asset.status === 'UPLOADED' || asset.status === 'TRANSCODING') {
        await this.assets.transitionStatus(assetId, asset.status, 'READY');
        this.logger.log(
          `Transcode job ${job.id}: transcoding disabled — asset ${assetId} marked READY (progressive)`,
        );
      }
