...
describe('MediaService.getPlaybackUrls', () => {
  ...
  beforeEach(() => {
    r2 = {
      getBucket: jest.fn().mockReturnValue('bilgim-media'),
    ...
    service = new MediaService(r2, assets, access, transcodingQueue);
  });
  ...
  describe('HLS playback (Req 8.3, 8.4)', () => {
    it('signs the master manifest plus every HLS variant playlist', async () => {
      ...
      // Each variant entry derives its key from the manifest path.
      for (const variant of HLS_VARIANTS) {
        ...
        expect(entry!.url).toBe(`https://r2/sign/${expectedKey}`);
      }
    });
  ...
  describe('authorization (Req 8.8)', () => {
    it('delegates to MediaAccessService.assertCanRead before signing', async () => {
      assets.findById.mockResolvedValue(baseAsset() as any);
      ...
      // `assertCanRead` runs before any signing.
      const accessOrder = access.assertCanRead.mock.invocationCallOrder[0]!;
      const signOrder = r2.signObjectGet.mock.invocationCallOrder[0]!;
      expect(accessOrder).toBeLessThan(signOrder);
    });
