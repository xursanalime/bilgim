...
describe('LiveController', () => {
  ...
  describe('POST /live/sessions/:lessonId/start', () => {
    it('forwards lessonId + teacher.sub to LiveService.startSession', async () => {
      ...
      const result = await controller.start(teacherUser, LESSON_ID);

      expect(service.startSession).toHaveBeenCalledWith(LESSON_ID, TEACHER_ID);
    ...
    it('returns the service result unchanged (idempotent path also works)', async () => {
      ...
      await expect(controller.start(teacherUser, LESSON_ID)).resolves.toBe(
        payload,
      );
    });
