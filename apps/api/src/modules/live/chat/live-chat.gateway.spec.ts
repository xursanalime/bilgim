...
function makePrisma(overrides: Partial<FakePrisma> = {}): FakePrisma {
  return {
    ...
    // Backs `nextRoomSeq` (per-room monotonic message counter — see
    // `ChatRoom.lastSeq` in schema.prisma).
    $queryRaw: jest.fn().mockResolvedValue([{ lastSeq: 1n }]),
    ...overrides,
  };
}
...
describe('LiveChatGateway.onMessage', () => {
  ...
  it('broadcasts to the lesson room and persists the message (Req 9.9)', async () => {
    ...
    expect(prisma.chatMessage.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: { roomId: 'room-1', seq: 1n, authorId: 'student-1', body: 'hello' },
      }),
    );
