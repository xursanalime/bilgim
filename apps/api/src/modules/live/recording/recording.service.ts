...
export class RecordingService {
  ...
  // ==================================================================
  // Read (lesson recordings listing)
  // ==================================================================

  /**
   * List all Recording rows for a lesson, newest first. Used by the
   * lesson page + dashboard to surface on-demand recordings (Req 6.2).
   * Read-only passthrough to the repository.
   */
  async listForLesson(lessonId: string) {
    return this.recordingRepository.listByLessonId(lessonId);
  }
  ...
  async finalizeRecording(
    ...
    // Integrity guard: a recorder may report "success" while the artefact is
    // not actually present in storage (e.g. the local/dev fake recorder, or a
    // real recorder whose upload silently failed). Creating a READY recording
    // + playable Attachment for a missing object yields a black "404" video
    // for teachers. Verify the object exists before committing success; if it
    // does not, finalize honestly as RECORDING_FAILED so the UI shows the
    // recording as unavailable instead of a broken player. Skipped when no R2
    // client is wired (unit tests), preserving their behaviour.
    if (this.r2) {
      let exists = false;
      try {
        exists = await this.r2.objectExists(result.uploadedKey);
      } catch (error) {
        this.logger.warn(
          `finalizeRecording: objectExists check failed for ${result.uploadedKey}: ${(error as Error).message}`,
        );
      }
      if (!exists) {
        this.logger.warn(
          `finalizeRecording: recorder reported success but object ${result.uploadedKey} is absent in storage for session ${sessionId}; marking RECORDING_FAILED`,
        );
        return this.applyFailure(
          session,
          'recording artefact missing in storage',
        );
      }
    }
