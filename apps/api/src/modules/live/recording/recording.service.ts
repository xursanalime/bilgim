...
export interface FinalizeRecordingOutcome {
  /** SKIPPED means the teacher opted out of recording — not a failure. */
  status: 'READY' | 'FAILED' | 'SKIPPED';
  /** Empty string when status === 'SKIPPED' (no Recording row was created). */
  recordingId: string;
...
export class RecordingService {
  ...
  async startRecording(sessionId: string): Promise<StartRecordingResult> {
    ...
    if (session.shouldRecord === false) {
      this.logger.log(
        `startRecording: session ${sessionId} opted out of recording; skipping`,
      );
      return { recorderRef: '', spawned: false };
    }
   ...
   *   - **skipped**: the teacher opted out (`shouldRecord === false`) —
   *     no Recording row, no status change, nothing enqueued.
   *
  ...
  async finalizeRecording(
    ...
    if (!session.recorderRef) {
      if (session.shouldRecord === false) {
        // Teacher opted out — no recorder was ever supposed to run, so
        // this isn't a failure. Leave the LiveSession status exactly as
        // `LiveService.endSession` set it (ENDED); don't create a
        // Recording row or flip to RECORDING_FAILED.
        this.logger.log(
          `finalizeRecording: session ${sessionId} was not recorded (teacher opted out); skipping`,
        );
        return {
          status: 'SKIPPED',
          recordingId: '',
