...
export class RecordingRepository {
  ...
  /**
   * List all Recording rows for a lesson, newest first. Includes
   * RECORDING (in-progress), READY and FAILED rows so the UI can surface
   * each state (processing / watch / failed). Read-only.
   */
  async listByLessonId(lessonId: string): Promise<Recording[]> {
