...
export class LiveController {
  ...
  /**
   * GET /live/sessions/active-for-me — Currently LIVE sessions for all
   * groups the calling STUDENT is enrolled in.
   * Used by the student dashboard "Hozir jonli!" banner.
   *
   * NOTE: declared BEFORE the parameterized `sessions/:lessonId` route so
   * Express matches this static path first. Otherwise `active-for-me` is
   * captured as a `:lessonId` value and crashes the UUID lookup in
   * `LessonAccessGuard`.
   */
  @Roles('STUDENT')
  @Get('sessions/active-for-me')
  async listActiveForMe(@CurrentUser() user: JwtPayload) {
    return this.liveService.listActiveSessionsForStudent(user.sub);
  }

  /**
   * GET /live/lessons/:lessonId/recordings — list a lesson's recordings
   * for on-demand viewing (Req 6.2). Gated by `LessonAccessGuard` so only
   * the owning TEACHER, an ADMIN auditor, or an APPROVED student can read.
   * Returns RECORDING/READY/FAILED rows newest-first.
   */
  @UseGuards(LessonAccessGuard)
  @Roles('TEACHER', 'STUDENT', 'ADMIN')
  @Get('lessons/:lessonId/recordings')
  async listLessonRecordings(
    @Param('lessonId', new ParseUUIDPipe()) lessonId: string,
  ) {
    return this.liveService.listLessonRecordings(lessonId);
  }
