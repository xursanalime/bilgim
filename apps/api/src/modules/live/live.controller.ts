...
} from './dto/start-live-session.dto';
import {
  JoinSessionResult,
  LiveService,
  LiveSessionWithSfu,
...
export class LiveController {
   ...
   * session (Req 9.1). `record` is the teacher's answer to the pre-start
   * "record this lesson?" prompt; omitted defaults to `true`.
   */
  @Roles('TEACHER')
  ...
  async start(
    ...
    @Body(new ZodValidationPipe(StartLiveSessionSchema)) dto: StartLiveSessionDto,
  ): Promise<LiveSessionWithSfu> {
    return this.liveService.startSession(lessonId, user.sub, dto.record);
  }
