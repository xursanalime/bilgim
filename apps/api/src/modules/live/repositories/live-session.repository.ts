...
export interface CreateActiveSessionInput {
   ...
   */
  roomId: string;
  /** Teacher's recording choice. Defaults true (see schema comment). */
...
export class LiveSessionRepository {
  ...
  async createActive(
    ...
    return client.liveSession.create({
      data: {
        ...
        status: 'LIVE',
        startedAt: new Date(),
        shouldRecord: input.shouldRecord ?? true,
      },
    });
  }
   ...
   *
   * `shouldRecord` is applied here (not just at row creation) because the
   * row may already exist as a `SCHEDULED` waiting-room session opened by
   * an early-joining STUDENT — before the teacher ever made a recording
   * choice. This is the teacher's actual decision point, so it overwrites
   * whatever default the row was created with.
   */
