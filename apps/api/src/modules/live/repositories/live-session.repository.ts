...
export class LiveSessionRepository {
  ...
  /**
   * Force a session into `RECORDING_FAILED` from any non-terminal state
   * (or from `ENDED`, when finalization fails after the API flipped the
   * session to ENDED). Used by `RecordingFinalizer` as the safety net that
   * guarantees a LiveSession always reaches a terminal status — Property 6
   * / content-protection Req 5.1, 5.3. Returns `true` when a row moved.
   */
  async transitionToRecordingFailed(
    id: string,
    tx?: Prisma.TransactionClient,
  ): Promise<boolean> {
    const client = tx ?? this.prisma;
    const result = await client.liveSession.updateMany({
      where: {
        id,
        status: {
          in: ['SCHEDULED', 'STARTING', 'LIVE', 'ENDED'] as LiveSessionStatus[],
        },
      },
      data: {
