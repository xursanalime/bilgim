...
export class LiveService {
   ...
   *
   * `shouldRecord` is the teacher's choice, made in the "start" dialog —
   * defaults true so callers that don't pass it (e.g. older clients)
   * keep the platform's original "every lesson is recorded" behavior.
   */
  async startSession(
    lessonId: string,
    teacherId: string,
    ...
    if (existing) {
      if (existing.status === 'SCHEDULED') {
        const promoted = await this.prisma.$transaction(async (tx) => {
          const transitioned = await this.liveSessionRepository.transitionToLive(existing.id, shouldRecord, tx);
          if (!transitioned) {
    ...
    try {
      const session = await this.prisma.$transaction(async (tx) => {
        const created = await this.liveSessionRepository.createActive(
          { lessonId, roomId: room.roomId, shouldRecord },
          tx,
        );
