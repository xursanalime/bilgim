...
describe('DiscoveryRepository', () => {
  ...
  describe('searchCourses', () => {
    ...
    it('filters courses by one or more CEFR levels (Req 14.2)', async () => {
      await repo.searchCourses({ pageSize: 20, cefrLevels: ['A2', 'B1'] });
      const where = prisma.course.findMany.mock.calls[0][0].where;
      expect(where.cefrLevel).toEqual({ in: ['A2', 'B1'] });
    });

    it('filters courses by examTrack slug (Req 14.3)', async () => {
      await repo.searchCourses({ pageSize: 20, examTrack: 'ielts' });
      const where = prisma.course.findMany.mock.calls[0][0].where;
      expect(where.examTrack).toBe('ielts');
    });

    it('does not apply any specialty-based filter (English-only refocus, Req 14.4)', async () => {
      await repo.searchCourses({ pageSize: 20 });
      const where = prisma.course.findMany.mock.calls[0][0].where;
      expect(where.teacher).toBeUndefined();
      expect(where).not.toHaveProperty('specialtyId');
    });

    it('selects cefrLevel and examTrack so discovery can expose them (Req 14.2/14.3)', async () => {
      await repo.searchCourses({ pageSize: 20 });
      const select = prisma.course.findMany.mock.calls[0][0].select;
      expect(select.cefrLevel).toBe(true);
      expect(select.examTrack).toBe(true);
    });
  ...
  describe('searchTeachers', () => {
    ...
    it('filters teachers by taughtCefrLevels (Req 14.2)', async () => {
      await repo.searchTeachers({ pageSize: 20, cefrLevels: ['B2', 'C1'] });
      const where = prisma.teacherProfile.findMany.mock.calls[0][0].where;
      expect(where.taughtCefrLevels).toEqual({ hasSome: ['B2', 'C1'] });
    });

    it('filters teachers by examTrackFocus slug (Req 14.3)', async () => {
      await repo.searchTeachers({ pageSize: 20, examTrack: 'ielts' });
      const where = prisma.teacherProfile.findMany.mock.calls[0][0].where;
      expect(where.examTrackFocus).toEqual({ has: 'ielts' });
    });

    it('does not apply any specialty-based filter (Req 14.4)', async () => {
      await repo.searchTeachers({ pageSize: 20 });
      const where = prisma.teacherProfile.findMany.mock.calls[0][0].where;
      expect(where).not.toHaveProperty('specialtyId');
    });
