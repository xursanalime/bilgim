...
function makeRepoFake(initialMessages: FakeMessage[] = []) {
  ...
  const repo = {
    ...
    listMessages: jest.fn(
      async (
        threadId: string,
        opts: { cursor?: Date; pageSize: number },
      ) => {
        const filtered = messages
          .filter((m) => m.deletedAt === null && m.roomId === threadId)
          .filter((m) =>
            opts.cursor ? m.createdAt.getTime() < opts.cursor.getTime() : true,
          )
...
describe('DmService', () => {
  ...
  describe('openThread', () => {
    ...
    it('blocks STUDENT ↔ STUDENT DMs even when both users are visible', async () => {
      const prisma = makePrismaFake({
        users: {
          [STUDENT_ID]: ACTIVE_STUDENT,
