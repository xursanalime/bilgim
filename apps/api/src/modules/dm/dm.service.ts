...
export interface DmMessage {
  id: string;
  threadId: string;
  /** Per-thread monotonic order, serialized as a string (JSON has no bigint). */
  seq: string;
  authorId: string;
  text: string;
  assetId: string | null;
...
export class DmService {
  ...
  async listThreads(
    ...
    // N+1 fix (Task 24.2 follow-up): one batched query for the whole
    // inbox's unread counts instead of a `count()` per room — see
    // `countUnreadBatch` for why a per-room `lastReadAt` cutoff rules
    // out a plain `count({ roomId: { in } })`.
    const unreadCountMap = await countUnreadBatch(
      this.prisma,
      kept.map(({ room }) => ({
        roomId: room.id,
        lastReadAt: readReceiptMap.get(room.id) ?? new Date(0),
      })),
      actor.userId,
    );
  ...
  async getUnreadCount(actor: DmActor): Promise<DmUnreadCount> {
    ...
    let totalUnread = 0;
    for (const count of unreadCountMap.values()) totalUnread += count;
   ...
   *  - STUDENT + STUDENT: allowed unconditionally (students may freely
   *    message each other outside of a group chat).
  ...
  private async assertMutuallyVisible(
    ...
    // STUDENT + STUDENT: allowed unconditionally — the only remaining
    // combination once ADMIN and TEACHER+TEACHER pairs have been
    // rejected above, besides TEACHER+STUDENT handled below.
  ...
  /** Cursor for `listThreads` — thread pagination stays ordered by last-activity timestamp, not `seq` (each thread has its own seq space). */
  ...
  /** Cursor for `listMessages` — see `ChatMessage.seq` for why message pagination is keyed on seq rather than `createdAt`. */
