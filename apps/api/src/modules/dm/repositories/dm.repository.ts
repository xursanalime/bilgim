...
export class DmRepository {
  ...
  /**
   * List messages in a thread, newest first. Pagination uses the
   * `createdAt` of the oldest already-seen message as cursor.
   */
  async listMessages(
    threadId: string,
    opts: { cursor?: Date | undefined; pageSize: number },
    tx?: Prisma.TransactionClient,
  ): Promise<DmMessageRow[]> {
    const client = tx ?? this.prisma;
   ...
   *
   * The implementation reads with `orderBy: createdAt desc`, deduping
   * in JS rather than `distinct on (roomId)` so the same code path
   * works against the SQLite-backed unit tests. With Postgres the
   * planner uses the `(roomId, createdAt)` index already covering
   * the existing `findLatestMessage` query.
   */
