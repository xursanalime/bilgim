...
import { CopyableCode } from '../../../../../../../../components/dashboard/copyable-code';

interface PageProps {
  params: { locale: string; courseId: string; groupId: string };
  /**
   * `?from=groups` is set by the groups overview. A group lives under a
   * course in the URL, so the back link always pointed at the course — which
   * dumped anyone who arrived from Guruhlar onto a page they had not visited.
   * The parameter survives a refresh, unlike browser history.
   */
...
export default async function GroupDetailPage({
  ...
  try {
    lessons = await serverApi.get<Lesson[]>(`/catalog/groups/${groupId}/lessons`);
  } catch (err) {
    if (!(err instanceof ServerApiError)) throw err;
  }

  const cameFromGroups = searchParams.from === 'groups';
  const backHref = cameFromGroups
    ? `/${locale}/groups`
    : `/${locale}/dashboard/courses/${courseId}`;
  const backLabel = cameFromGroups ? 'Barcha guruhlar' : course.title;
      ...
      <header className="group relative overflow-hidden rounded-[2.5rem] border border-rim bg-white p-8 shadow-soft sm:p-10 lg:p-12">
        ...
        <div className="relative z-10 flex flex-col justify-between gap-8 lg:flex-row lg:items-end">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-sm font-black text-ink-strong tabular-nums">
                    ...
                    {group.capacity != null && (
                      <span className="font-bold text-ink-faint"> / {group.capacity}</span>
                    )}
                  </p>
                </div>
              </div>
              ...
              {group.startsOn && (
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-black text-ink-strong">
                      {new Date(group.startsOn).toLocaleDateString('uz-UZ')}
                    </p>
                  </div>
                </div>
              )}

              {group.joinCode && <CopyableCode code={group.joinCode} />}
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Link
              href={`/${locale}/dashboard/courses/${courseId}/groups/${groupId}/journal`}
              className="inline-flex h-11 items-center gap-2 rounded-2xl border border-rim bg-white px-5 text-sm font-bold text-ink-strong shadow-sm transition-all hover:bg-tint hover:border-teal/20 active:scale-[0.98]"
            >
