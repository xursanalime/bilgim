 ...
 *
 * Authentication is cookie-based: `withCredentials: true` makes the browser
 * attach the httpOnly `bilgim_access_token` cookie to the WebSocket
 * handshake, and the API's `authenticateSocket` reads it from there. The
 * token is never handled in JS, matching the REST BFF model. The gateway's
 * CORS config must allow credentials from the web origin (it does).
 */
