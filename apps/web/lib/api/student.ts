...
export interface StudentGroup {
  ...
  status: string;
  /**
   * Server-authoritative Download_Permission (Content Protection Req 2.1 /
   * 4.5): whether entitled students may download this group's protected
   * lesson materials. The backend always surfaces this on Group DTOs and
   * coerces a missing column to `false`, so the client treats a missing value
   * as "downloads disabled" (fail-closed). Display-only — the API enforces the
   * real gate when streaming bytes.
   */
...
export interface ScheduleResponse {
  ...
  /** Concrete RRULE-expanded occurrences (UTC) within the server window. */
