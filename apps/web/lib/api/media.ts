...
export const mediaApi = {
  ...
  /**
   * Metadata + signed playback URL for many assets in one round-trip —
   * use this instead of calling `getAssetMetadata` + `getPlaybackUrl`
   * per attachment when rendering a page of chat history.
   */
  getAssetsBatch(assetIds: string[]) {
    return apiClient.post<MediaAssetBatchResult[]>('/media/assets/batch-urls', {
      assetIds,
    });
  },
};
