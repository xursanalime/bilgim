...
function readStoredUser(): StoredUser | null {
  ...
  const direct = parseStoredUser(raw);
  if (direct) return direct;

  // Cookies written before the login route stopped double-encoding this
  // value (it manually called `encodeURIComponent` on top of Next.js's own
  // cookie-value encoding) still linger in signed-in browsers as one extra
  // layer of `%xx` escaping — `raw` above is that leftover encoded JSON, not
  // valid JSON yet. Undo the extra layer here so those existing sessions
  // self-heal on next read instead of silently reading as "no role" until
  // the user manually logs out and back in.
  try {
    return parseStoredUser(decodeURIComponent(raw));
  } catch {
    return null;
  }
}
