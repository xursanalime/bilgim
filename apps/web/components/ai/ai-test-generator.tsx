    ...
    <div className="rounded-2xl border border-blue/20 bg-blue/[0.03] overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-black text-cream">AI bilan test yaratish</p>
            <p className="text-[11px] text-cream-dim">Matn kiriting — AI avtomatik savollar tuzadi</p>
          </div>
        </div>
        {expanded ? (
      ...
      {expanded && (
        <div className="px-5 pb-5 space-y-5 border-t border-white/[0.06]">
          <div className="pt-4 space-y-1">
            <label className="font-mono text-[10px] uppercase tracking-[0.15em] text-cream-dim">
            ...
            <textarea
              ...
              className="w-full rounded-2xl border border-ink-line bg-white/[0.04] px-4 py-3 text-sm text-cream placeholder-cream-dim/50 outline-none focus:border-blue/40 focus:bg-white/[0.06] resize-none"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div className="space-y-1">
              ...
              <select
                ...
                className="w-full rounded-xl border border-ink-line bg-white/[0.04] px-3 py-2 text-sm text-cream outline-none focus:border-blue/40"
              >
                {[3, 5, 8, 10, 15].map((n) => (
                  <option key={n} value={n} className="bg-ink-surface">
                    {n} ta
                  </option>
                ))}
              </select>
            ...
            <div className="space-y-1">
              ...
              >
                {Object.entries(DIFFICULTY_LABELS).map(([k, v]) => (
                  <option key={k} value={k} className="bg-ink-surface">
                    {v}
                  </option>
                ))}
              </select>
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                className="w-full rounded-xl border border-ink-line bg-white/[0.04] px-3 py-2 text-sm text-cream outline-none focus:border-blue/40"
              >
                <option value="uz" className="bg-ink-surface">O'zbek</option>
                <option value="ru" className="bg-ink-surface">Русский</option>
                <option value="en" className="bg-ink-surface">English</option>
              </select>
            </div>
              ...
              <div className="flex flex-col gap-1">
                {Object.entries(TYPE_LABELS).map(([k, v]) => (
                  <label key={k} className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-[11px] text-cream-dim">{v}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-3 border-t border-white/[0.06] pt-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest text-cream-dim">
                ...
                {result.questions.map((q, i) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <p className="text-sm text-cream leading-relaxed">{q.prompt}</p>
                    </div>
                    ...
                    {q.explanation && (
                      <p className="mt-2 ml-14 text-[11px] text-cream-dim italic">{q.explanation}</p>
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full rounded-xl border border-ink-line bg-white/[0.04] px-4 py-3 text-sm text-cream placeholder-cream-dim/50 outline-none focus:border-blue/40 resize-none"
                />
              </div>
              ...
              <button
                type="button"
                onClick={handleApply}
                className="flex items-center gap-2 rounded-xl bg-green/20 border border-green/30 text-green px-5 py-2.5 text-sm font-black transition-all hover:bg-green/30 active:scale-95"
              >
