...
export function AiGradingPanel({
  ...
  return (
    <div className="rounded-2xl border border-rim bg-canvas overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-rim">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-black text-ink-strong">AI Baholash</p>
          {result && (
            <span className={cn(
      ...
      <div className="p-4 space-y-4">
        ...
        {result && (
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-xl bg-tint border border-rim p-3 text-center">
              <p className="text-[10px] font-black uppercase tracking-widest text-ink-soft mb-1">AI Ball</p>
              <p className="text-2xl font-black text-ink-strong">
                {Math.round(result.score * totalPoints)}
                <span className="text-sm text-ink-soft font-medium">/{totalPoints}</span>
              </p>
            </div>
            <div className="rounded-xl bg-tint border border-rim p-3 text-center">
              <p className="text-[10px] font-black uppercase tracking-widest text-ink-soft mb-1">AI Ehtimol</p>
              <div className="flex items-center justify-center gap-1.5">
                <Shield className={cn(
                  'h-4 w-4',
                  result.aiFlagged ? 'text-red' : 'text-teal',
                )} />
                <p className="text-2xl font-black text-ink-strong">
        ...
        {result && expanded && (
          <div className="space-y-3">
            {result.summary && (
              <div className="rounded-xl bg-tint border border-rim p-3">
                <p className="text-[10px] font-black uppercase tracking-widest text-ink-soft mb-2 flex items-center gap-1.5">
                  <BarChart3 className="h-3 w-3" /> AI Xulosa
                </p>
                <p className="text-sm text-ink-strong/80 leading-relaxed">{result.summary}</p>
              </div>
            )}

            {result.highlights.length > 0 && (
              <div className="space-y-2">
                <p className="text-[10px] font-black uppercase tracking-widest text-ink-soft">Belgilangan joylar</p>
        ...
        <button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center gap-2 rounded-xl border border-rim bg-tint px-4 py-2.5 text-sm font-black text-ink-soft hover:bg-soft hover:text-ink-strong transition-all disabled:opacity-50 disabled:cursor-not-allowed active:scale-95"
        >
          {loading ? (
            <>
