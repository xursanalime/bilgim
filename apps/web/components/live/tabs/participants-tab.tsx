        ...
        {participants.map((p) => (
          <div key={p.socketId} className="group flex items-center justify-between gap-3 p-3 rounded-2xl bg-tint/50 border border-rim hover:bg-tint transition-all">
            ...
            <div className="flex items-center gap-1">
              <div className={cn("p-1.5 rounded-lg", p.isMicOn ? "text-blue bg-blue/5" : "text-red bg-red/5")}>
                {p.isMicOn ? <Mic className="h-3.5 w-3.5" /> : <MicOff className="h-3.5 w-3.5" />}
              </div>
              <div className={cn("p-1.5 rounded-lg", p.isCamOn ? "text-blue bg-blue/5" : "text-red bg-red/5")}>
              ...
              {isTeacher && p.role !== 'TEACHER' && (
                <button
                  ...
                  className="p-1.5 rounded-lg text-ink-faint outline-none hover:bg-red/10 hover:text-red focus-visible:ring-2 focus-visible:ring-red focus-visible:ring-offset-2 transition-all opacity-0 group-hover:opacity-100 focus-visible:opacity-100"
                  title="Darsdan chetlatish"
                  aria-label={`${p.name}ni darsdan chetlatish`}
                >
