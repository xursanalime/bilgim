...
export function GamificationOnboarding() {
  ...
  // Custom (non-Radix) overlay: trap focus, close on Esc, restore focus on close.
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-ink-strong/40 p-4 backdrop-blur-sm">
      <motion.div
        ref={dialogRef}
        role="dialog"
        aria-modal="true"
        aria-labelledby="gamification-onboarding-title"
        tabIndex={-1}
        initial={{ opacity: 0, scale: 0.9, y: 20 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        className="relative w-full max-w-md overflow-hidden rounded-[2rem] bg-white shadow-2xl outline-none"
      >
        <button
          type="button"
          onClick={handleClose}
          aria-label="Yopish"
          className="absolute right-4 top-4 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-tint text-ink-faint outline-none transition-colors hover:bg-rim hover:text-ink-strong focus-visible:ring-2 focus-visible:ring-blue"
        >
        ...
        <div className="flex flex-col items-center p-8 text-center pt-12">
          ...
          >
            <Icon className={cn("h-12 w-12", step.color)} aria-hidden="true" />
          </motion.div>

          <h2
            id="gamification-onboarding-title"
            className="mb-3 font-display text-2xl font-extrabold text-ink-strong"
          >
            {step.title}
          </h2>
          ...
          <div className="mt-8 flex w-full items-center justify-between">
            ...
            <button
              type="button"
              onClick={handleNext}
              className="flex items-center gap-2 rounded-2xl bg-blue px-6 py-3 text-sm font-bold text-white shadow-blue-soft outline-none transition-all hover:bg-blue-600 focus-visible:ring-2 focus-visible:ring-blue focus-visible:ring-offset-2 active:scale-95"
            >
