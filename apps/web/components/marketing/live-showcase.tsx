        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between border-t border-white/[0.07] bg-ink/60 px-5 py-3">
              ...
              {/* Decorative mockup control — not a real action. Hidden from
                  keyboard + a11y tree so it isn't a dead tab stop. */}
              <button
                aria-hidden="true"
                tabIndex={-1}
...
function ControlButton({
  ...
  return (
    // Decorative mockup control — performs no action. Removed from the tab
    // order and the accessibility tree so keyboard users don't land on a
    // dead, unlabeled stop inside the visual.
    <button
      aria-hidden="true"
      tabIndex={-1}
