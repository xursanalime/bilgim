...
export function ResetPasswordForm({
  ...
  const onSubmit = async ({ password }: Values) => {
    if (!token) {
      setServerError('Tiklash havolasi yaroqsiz.');
      return;
    }
    setServerError(null);
    try {
  ...
  if (!token) {
    return (
      <div className="space-y-6 text-center">
        ...
        <h2 className="text-2xl font-bold text-white-strong">Havola topilmadi</h2>
  ...
  if (success) {
    return (
      <div className="space-y-6 text-center">
        ...
        <h2 className="text-2xl font-bold text-white-strong">
          Parol o&apos;zgartirildi 🎉
        </h2>
        <p className="text-white-soft">
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      {serverError && (
        <div className="rounded-2xl border border-red/30 bg-red-tint p-4 text-sm text-red">
          {serverError}
        </div>
      )}
      ...
      <FormField
        ...
        rightIcon={
          <button
            ...
            className="pointer-events-auto text-white-soft hover:text-white-strong"
            tabIndex={-1}
          >
            {showPassword ? (
              <svg
