...
export function StudentSchedule({ locale }: StudentScheduleProps) {
  ...
  const calendar = useMemo<CalendarEntry[]>(() => {
    ...
    enrollments.forEach((enr, idx) => {
      ...
      const upcoming = scheduleQueries[idx]?.data?.occurrences ?? [];
      for (const occ of upcoming) {
        const when = Date.parse(occ.start);
        if (Number.isNaN(when)) continue;
        if (when < Date.now() - 60 * 60 * 1000) continue; // skip already-finished
    ...
    rows.sort(
      (a, b) =>
        Date.parse(a.occurrence.start) - Date.parse(b.occurrence.start),
    );
    return rows;
  ...
  const grouped = useMemo(() => {
    ...
    for (const entry of calendar) {
      const dayKey = formatDayKey(entry.occurrence.start);
            ...
            <section key={dayKey} className="space-y-6">
              ...
              <div className="grid gap-4">
                {entries.map((entry, idx) => (
                  <Link
                    key={`${entry.groupId}-${entry.occurrence.start}-${idx}`}
                    ...
                    <div className="flex items-center gap-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 flex items-center gap-2 text-xs font-medium text-ink-faint">
                          ...
                          <span>{formatTimeRange(entry.occurrence.start, entry.occurrence.end)}</span>
                        </div>
                      </div>
                    </div>
