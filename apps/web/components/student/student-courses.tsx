...
export function StudentCourses({ locale }: StudentCoursesProps) {
  ...
  if (enrollments.length === 0 && requests.length === 0) {
    return (
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-10 pb-12">
            ...
            return (
              ...
              >
                ...
                <div className="mt-8 flex items-center justify-between border-t border-rim pt-5">
                  <span className="inline-flex rounded-full bg-green-tint px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-green border border-green/10">
                    Faol
                  </span>
      ...
      {requests.length > 0 && (
        <EnrollmentStatus locale={locale} heading="Aʼzolik soʻrovlari" />
      )}
    </div>
  );
}
