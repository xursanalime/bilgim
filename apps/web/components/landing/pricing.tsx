      ...
      <div className="container-page">
        <SectionHeader
          ...
          title={
            <>
              Sizga mos{' '}
              <span className="italic text-hero-gradient">narx</span> tanlang
            </>
          }
    ...
    >
      ...
      <div className="relative">
        <div className="mt-3 flex items-baseline gap-1.5">
          <span
            className={`text-2xl font-extrabold ${popular ? 'text-blue' : 'text-ink-strong'}`}
        ...
        <Link
          ...
          className={`mt-7 inline-flex w-full items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-bold transition-all active:scale-[0.97] ${
            popular
              ...
              : 'border border-rim-2 bg-canvas text-white hover:border-rim-2 hover:bg-soft'
          }`}
          style={
            popular
              ? {
        ...
        <ul className="mt-7 space-y-2.5 border-t border-rim pt-7">
          {features.map((f) => (
            ...
            >
              <Check className={popular ? 'text-blue' : 'text-teal'} />
              <span>{f}</span>
            </li>
          ))}
        </ul>
