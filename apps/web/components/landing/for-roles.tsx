...
function RoleCard({
  ...
  const colors = {
    blue: { hex: tokens.brand.primary, bg: 'bg-blue-tint', text: 'text-blue', glow: 'rgba(0, 113, 227, 0.3)' },
    green: { hex: tokens.semantic.success, bg: 'bg-green-tint', text: 'text-green', glow: 'rgba(52, 199, 89, 0.3)' },
    purple: { hex: tokens.ai.accent, bg: 'bg-purple-tint', text: 'text-purple', glow: 'rgba(175, 82, 222, 0.3)' },
  }[tone];
          ...
          {features.map((f) => (
            ...
            >
              <Check
                ...
                style={{ color: highlighted ? colors.hex : tokens.semantic.success }}
                strokeWidth={3}
              />
              <span>{f}</span>
            </li>
        ...
        <Link
          ...
          className={`mt-7 inline-flex w-full items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-bold transition-all active:scale-[0.97] ${
            highlighted
              ...
              : 'border border-rim bg-canvas text-ink-strong hover:border-rim-2 hover:bg-tint'
          }`}
          style={
            highlighted
              ? {
