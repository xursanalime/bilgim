          ...
          <div className="flex flex-col gap-8 p-8 md:flex-row md:items-center">
            ...
            <div className="flex w-full max-w-sm flex-col gap-6">
              ...
              <div className="flex items-center gap-3 rounded-xl border border-rim bg-tint p-4">
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-teal-tint">
                  <Wifi className="h-4 w-4 text-teal" />
                </div>
                <div>
                  <p className="text-xs font-bold text-ink-strong">Aloqa holati</p>
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-teal">Tayyor</p>
                </div>
              </div>
...
function ConnectionOverlay() {
  ...
  return (
    <div className="absolute inset-0 z-[300] flex items-center justify-center bg-tint-strong/60 backdrop-blur-sm">
...
function InnerRoom({ lessonId, role, onLeave }: InnerRoomProps) {
  ...
  return (
    <div className="h-screen w-full bg-[rgb(var(--bg-base))] text-ink-strong overflow-hidden font-sans relative">
      ...
      {/* Reserved top/bottom padding must clear LiveTopBar/LiveControlBar's
          real rendered height at each breakpoint — see the matching
          comment in live-sidebar.tsx, which reserves the same amounts. */}
      <main className="flex h-full flex-col pt-[96px] pb-[64px] relative z-20 sm:pt-[72px] sm:pb-[80px]">
            ...
            <div className="relative h-full w-full overflow-hidden rounded-[1.25rem] bg-[#111118]">
              ...
              {!showWhiteboard && (
                <div className={`absolute bottom-3 left-3 flex items-center gap-1.5 backdrop-blur-md px-2.5 py-1 rounded-full border z-10 ${isWaiting ? 'bg-orange/20 border-orange/30' : 'bg-black/60 border-white/10'}`}>
                  <span className={`h-1.5 w-1.5 rounded-full ${isWaiting ? 'bg-orange' : 'bg-teal animate-pulse'}`} />
...
function VideoLayout() {
  ...
  if (screenTrack) {
    // Ekran ulashganda ekran butun kadrni egallaydi (Loom/OBS uslubida);
    // ulashayotgan ishtirokchining o'z kamerasi pastki o'ng burchakda
    // dumaloq "bubble" sifatida chiqadi — avvalgi teng bo'lingan
    // ekran/kamera qatlamlari o'rniga.
    const presenterCamera = cameraTracks.find(
      (t) => t.participant.identity === screenTrack.participant.identity,
    );
    const otherCameras = cameraTracks.filter(
      (t) => t.participant.identity !== screenTrack.participant.identity,
    );

    return (
      <div className="relative h-full w-full overflow-hidden rounded-2xl bg-black border border-white/5">
        ...
        {presenterCamera && (
          <div className="absolute bottom-4 right-4 h-28 w-28 overflow-hidden rounded-full border-4 border-white/90 shadow-2xl sm:h-36 sm:w-36">
            <ParticipantTile
              trackRef={presenterCamera}
        ...
        {/* Boshqa ishtirokchilarning kamerasi (agar bo'lsa) — dumaloq
            bubble'ning ustidan, ekranning chap tomonida kichik qator. */}
        {otherCameras.length > 0 && (
          <div className="absolute bottom-4 left-4 h-20 max-w-[60%]">
            <CarouselLayout tracks={otherCameras} className="h-full gap-2">
...
export function LiveRoom({ token, serverUrl, lessonId, role, onLeave, returnLabel = 'Dashboardga qaytish' }: LiveRoomProps) {
  ...
  const roomOptions = React.useMemo<RoomOptions>(() => {
    ...
    return {
      ...
      publishDefaults: {
        simulcast: true,
        // Top layer must match `videoCaptureDefaults` below — the room
        // recording (LiveKitEgressRecorderAdapter) renders at 1080p, and
        // capping the actual camera capture (and therefore every
        // simulcast layer) at 720p meant the recording upscaled a 720p
        // source into a 1080p frame — soft/blurry, not genuinely higher
        // quality. Capturing at 1080p natively (falls back gracefully to
        // whatever a lower-end webcam actually supports) fixes that.
