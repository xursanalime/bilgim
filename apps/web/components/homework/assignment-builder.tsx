...
export function AssignmentBuilder({ locale }: AssignmentBuilderProps) {
  ...
  async function handleSubmit() {
    ...
    if (modules.length === 0) {
      setSubmitError('Kamida bitta modul qo\u2018shing.');
      return;
    }
  ...
  return (
    <div className="mx-auto max-w-4xl pb-32">
      ...
      <div className="space-y-5">
        ...
        <Card step={2} title="Asosiy ma'lumot" subtitle="Topshiriq nomi va muddati">
          <div className="space-y-4">
            ...
            <FieldGroup label="Ko'rsatma (ixtiyoriy)">
              <textarea
                rows={3}
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                placeholder="Talabaga ko'rsatiladigan qo'shimcha ko'rsatmalar..."
                className={inputCls}
              />
            </FieldGroup>
        ...
        >
          ...
          {!groupId ? (
            ...
            <div className="flex items-start gap-3 rounded-2xl border border-orange/20 bg-orange/5 px-4 py-3.5">
              <span className="text-xl mt-0.5">⚠️</span>
              <p className="text-sm text-ink-soft">
                Bu guruh uchun hech bir modul yoqilmagan. Guruh sozlamalaridan kerakli modullarni yoqing.
              </p>
            </div>
          ) : (
      ...
      {/* ── Sticky action bar ── */}
      <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-rim bg-white/80 backdrop-blur-xl px-6 py-4">
        <div className="mx-auto max-w-4xl flex items-center justify-between gap-4">
          <div className="text-sm text-ink-soft hidden sm:block">
            {modules.length === 0
              ? 'Kamida 1 ta modul qo\'shing'
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        ...
        {questions.map((q, i) => (
          <div key={i} className="rounded-xl border border-white/[0.08] p-3 space-y-2">
