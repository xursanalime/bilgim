...
function SpeakingTaskCard({
  ...
  const [resolvedTeacherUrl, setResolvedTeacherUrl] = useState<string | null>(null);
  const teacherBlobRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (!task.teacherAudioUrl) return;
    let cancelled = false;
    fetch(task.teacherAudioUrl, { credentials: 'omit' })
      .then((res) => res.blob())
      .then((blob) => {
        if (cancelled) return;
        const url = URL.createObjectURL(blob);
        teacherBlobRef.current = url;
        setResolvedTeacherUrl(url);
      })
      .catch(() => {
        if (!cancelled) setResolvedTeacherUrl(task.teacherAudioUrl ?? null);
      });
    return () => {
      cancelled = true;
      if (teacherBlobRef.current) {
        URL.revokeObjectURL(teacherBlobRef.current);
        teacherBlobRef.current = null;
      }
    };
      ...
      {(showTarget || showText) && (
        <div className="rounded-xl bg-white/[0.04] border border-white/[0.06] px-4 py-3">
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            <button type="button" onClick={speakTarget} aria-label="Namuna talaffuzni tinglash" className="text-white/30 hover:text-white/60 transition-colors shrink-0">
