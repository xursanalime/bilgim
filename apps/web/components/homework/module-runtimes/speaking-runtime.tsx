...
function SpeakingTaskCard({
  ...
  const scoreColor =
    score >= 80 ? 'text-teal' : score >= 50 ? 'text-orange' : 'text-red';
  const scoreLabel =
    score >= 80 ? "A'lo!" : score >= 50 ? "Yaxshi, lekin davom eting" : "Ko'proq mashq qiling";

  return (
    <div className="rounded-2xl border border-rim bg-tint p-5 space-y-5">
      <p className="text-sm font-semibold text-ink-strong">{task.prompt}</p>
      ...
      {/* Target text */}
      {(showTarget || showText) && (
        <div className="rounded-xl bg-tint border border-rim px-4 py-3">
          <p className="text-[10px] font-black uppercase tracking-widest text-ink-faint mb-1">Aytish kerak</p>
          <div className="flex items-start justify-between gap-2">
            <p className="text-sm text-ink-strong leading-relaxed">{task.targetText}</p>
            <button onClick={speakTarget} className="text-ink-faint hover:text-ink-soft transition-colors shrink-0">
      ...
      {task.hint && !showText && !showTarget && (
        <button
          ...
          className="text-xs font-bold text-ink-faint hover:text-ink-soft transition-colors"
        >
          Matnni ko'rsatish
        </button>
      )}
      ...
      {editable && (
        <div className="flex items-center gap-3">
          ...
          {(state === 'done' || state === 'error') && (
            <button
              onClick={reset}
              className="flex items-center gap-2 rounded-xl border border-rim px-4 py-2 text-sm font-bold text-ink-soft hover:text-ink-strong hover:border-blue/30 transition-all"
            >
      ...
      {state === 'done' && transcript && (
        <div className="space-y-3">
          <div className="rounded-xl bg-tint border border-rim px-4 py-3">
            <p className="text-[10px] font-black uppercase tracking-widest text-ink-faint mb-1">Siz aytdingiz</p>
            <p className="text-sm text-ink-strong/90 italic">"{transcript}"</p>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1 h-2 rounded-full bg-soft overflow-hidden">
              <div
                className={cn('h-full rounded-full transition-all duration-500',
                  score >= 80 ? 'bg-teal' : score >= 50 ? 'bg-orange' : 'bg-red'
                )}
                style={{ width: `${score}%` }}
              />
            </div>
            <div className="text-right">
              ...
              <p className="text-[10px] font-bold text-ink-faint">{scoreLabel}</p>
            </div>
          </div>
        </div>
      )}
...
export function SpeakingRuntime({ config, initialAnswer, editable, onChange }: Props) {
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-xl border border-blue/20 bg-blue/5 px-4 py-3 text-xs text-ink-strong/80">
