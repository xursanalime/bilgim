...
function SentenceItem({
  ...
  return (
    <div className={cn(
      ...
      checked
        ? correct ? 'border-green/25 bg-green/5' : 'border-red/25 bg-red/5'
          ...
          return mode === 'choose' && sentence.wordBank ? (
            <button
              ...
              className={cn(
                ...
                val
                  ? checked
                    ? correct ? 'border-green/40 bg-green/15 text-green' : 'border-red/40 bg-red/15 text-red'
                    : 'border-blue/40 bg-blue/15 text-blue cursor-pointer hover:bg-blue/20'
                  : 'border-dashed border-white/[0.20] text-white/20',
            ...
            <input
              ...
              className={cn(
                'w-24 rounded-xl border bg-white/[0.04] px-3 py-1 text-sm font-bold text-center outline-none transition-colors',
                checked
                  ? correct ? 'border-green/40 text-green' : 'border-red/40 text-red'
                  : 'border-white/[0.15] text-white focus:border-blue/40',
                (!editable || checked) && 'cursor-not-allowed'
              )}
            />
          );
          ...
          {availableBank.map((word) => {
            ...
            return (
              <button
                ...
                className="rounded-xl border border-white/[0.10] bg-white/[0.04] px-3 py-1.5 text-sm font-bold text-white/70 hover:border-blue/40 hover:text-blue hover:bg-blue/10 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
              >
                {word}
              </button>
            );
      ...
      {checked && (
        <div className={cn(
          ...
          <div>
            {correct
              ? <p className="font-bold text-green">To'g'ri!</p>
              : <p className="font-bold text-white">
                  To'g'ri javob: <span className="text-green">{sentence.answers.join(', ')}</span>
                </p>
            }
          </div>
        </div>
        ...
        <div className="flex gap-2">
          ...
          {(checked || values.some((v) => v)) && (
            <button
              type="button"
              onClick={reset}
              className="flex items-center gap-1.5 rounded-xl border border-white/[0.08] px-3 py-2 text-sm font-bold text-white/40 hover:text-white/60 transition-all"
            >
