...
function WordTooltipPopup({
  ...
  // Custom (non-Radix) overlay: trap focus, close on Esc, restore focus to
  // the word that opened it (Req 7.1, 7.2, 7.3).
  ...
  return (
    <div
      ...
      style={style}
      role="dialog"
      aria-label={`"${tooltip.word}" so'zi tarjimasi`}
...
function ClickablePassage({
  ...
  return (
    <div className="space-y-3 whitespace-pre-wrap text-sm leading-[1.9] text-cream/90 select-text">
        ...
        return (
          <span key={si}>
            {words.map((token, wi) => {
              ...
              if (!bare || bare.length < 2) return <span key={wi}>{token}</span>;
              return (
                <button
                  type="button"
                  key={wi}
                  onClick={(e) => handleWordClick(token, sentence, e)}
                  className="inline cursor-pointer rounded px-0.5 align-baseline transition-colors hover:bg-blue/20 hover:text-white active:bg-blue/30"
...
export function ReadingRuntime({
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-5">
      {/* Passage */}
      <article className="rounded-2xl border border-white/[0.07] bg-white/[0.03] p-5 lg:col-span-2 relative">
        ...
        <ClickablePassage text={config.passage} onWordClick={handleWordClick} />
      </article>
...
function ReadingQuestionItem({
  ...
  return (
    <div className="rounded-2xl border border-white/[0.07] bg-ink-surface p-4">
      <p className="text-sm font-semibold text-cream">
        <span className="mr-2 font-mono text-[11px] text-cream-dim">{index}.</span>
        {question.prompt}
      </p>
      ...
      <div className="mt-3 space-y-2">
              ...
              return (
                <label
                  ...
                  className={`flex cursor-pointer items-center gap-3 rounded-xl border px-3 py-2 text-sm transition-colors ${
                    checked
                      ? 'border-accent2-500/40 bg-accent2-500/10 text-cream'
                      : 'border-white/[0.05] bg-white/[0.02] text-cream-dim hover:border-accent2-500/30 hover:text-cream'
                ...
                >
                  <input
                    ...
                    className="h-4 w-4 accent-accent2-500"
                  />
                  <span>{choice}</span>
                </label>
              );
        ...
        {question.kind === 'short' ? (
          <textarea
            ...
            className="w-full rounded-2xl border border-ink-line bg-white/[0.04] px-4 py-3 text-sm text-cream placeholder-cream-dim/60 outline-none focus:border-accent2-500/60 focus:bg-white/[0.06] disabled:cursor-not-allowed disabled:opacity-70"
          />
        ) : null}
      </div>
    </div>
