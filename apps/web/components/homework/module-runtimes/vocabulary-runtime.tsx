  ...
  if (mode === 'results') {
    return (
      <div className="flex flex-col items-center py-8 space-y-6">
        <div className="h-20 w-20 rounded-[2rem] bg-gradient-to-br from-blue-400/20 to-blue/20 border border-teal/20 flex items-center justify-center">
        ...
        <button
          onClick={restart}
          className="flex items-center gap-2 rounded-2xl border border-rim px-5 py-2.5 text-sm font-bold text-white/60 hover:text-white hover:border-rim transition-all"
        >
    ...
    <div className="space-y-5">
          ...
          {cards.map((c, i) => (
            <div
              ...
              className={cn(
                ...
                known.includes(c.id) ? 'bg-teal' :
                unknown.includes(c.id) ? 'bg-orange/60' :
                i === index ? 'bg-blue' : 'bg-soft'
              )}
            />
          ))}
        </div>
      ...
      >
        ...
        >
          {/* Front */}
          <div
            className="rounded-3xl border border-rim bg-gradient-to-br from-[#14141f] to-[#1a1a2e] p-8 min-h-[220px] flex flex-col items-center justify-center text-center"
      ...
      {editable && flipped && (
        ...
        >
          ...
          <button
            onClick={markKnown}
            className="flex-1 flex items-center justify-center gap-2 rounded-2xl border border-teal/30 bg-teal/10 py-3.5 text-sm font-black text-teal hover:bg-teal/20 transition-all active:scale-95"
          >
      ...
      <div className="flex items-center justify-between">
        <button
          onClick={prev}
          disabled={index === 0}
          className="h-10 w-10 rounded-xl border border-rim flex items-center justify-center text-white/40 hover:text-white hover:border-rim transition-all disabled:opacity-20"
        >
