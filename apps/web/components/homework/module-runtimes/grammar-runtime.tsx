...
export function GrammarRuntime({ config, initialAnswer, editable, onChange }: Props) {
  ...
  return (
    <div className="space-y-6">
      {/* Phase tabs */}
      <div className="flex rounded-2xl border border-rim bg-tint p-1 gap-1">
      ...
      {phase === 'learn' && (
        <div className="space-y-5">
          <div className="rounded-2xl border border-blue/20 bg-blue/5 p-5">
            ...
            {config.rule.formula && (
              <div className="mt-4 rounded-xl bg-soft border border-rim px-4 py-3">
          ...
          <div className="space-y-3">
            ...
            {config.rule.examples.map((ex, i) => (
              <div key={i} className="rounded-xl border border-rim bg-tint p-4 space-y-2">
              ...
              >
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          resp?.value === opt
                            ...
                            : 'border-rim text-white/50 hover:border-rim hover:text-white',
                ...
                {(ex.type === 'fill_blank' || ex.type === 'transformation' || ex.type === 'error_correction') && (
                  <input
                    ...
                    className={cn(
                      'w-full rounded-xl border bg-tint px-4 py-2.5 text-sm text-white placeholder-white/20 outline-none transition-colors',
                      checked
                        ? correct
                          ? 'border-teal/40'
                          : 'border-red/40'
                        : 'border-rim focus:border-blue/40'
                    )}
                  />
                )}
                    ...
                    <div className="space-y-1">
                      {!correct && (
                        <p className="font-bold text-white">
                          To'g'ri javob: <span className="text-teal">{ex.answer}</span>
                        </p>
                      )}
                      {correct && <p className="font-bold text-teal">Ajoyib! To'g'ri!</p>}
                      {ex.explanation && (
