...
import {
  AudioTaskRuntime,
  type AudioTaskRuntimeConfig,
  type AudioTaskRuntimeAnswer,
  type AudioSubmissionInfo,
} from './audio-task-runtime';
import {
  SpellingRuntime,
  type SpellingRuntimeConfig,
  type SpellingRuntimeAnswer,
} from './spelling-runtime';
import {
  GapFillRuntime,
...
interface ModuleRendererProps {
  ...
  /** AI-relevant submission slice — used by audio modules (Req 10.4). */
...
export function ModuleRenderer({
  ...
  switch (module.type) {
    ...
    case 'SPELLING': {
      const config = (module.configJson ?? {}) as SpellingRuntimeConfig;
      return (
        <SpellingRuntime
          config={config}
          initialAnswer={(initialAnswer as SpellingRuntimeAnswer | null) ?? null}
          editable={editable}
          onChange={(a) => onChange(a)}
        />
      );
    ...
    case 'SPEAKING':
    case 'PRONUNCIATION': {
      const config = (module.configJson ?? {}) as AudioTaskRuntimeConfig;
      return (
        <AudioTaskRuntime
          config={config}
          initialAnswer={(initialAnswer as AudioTaskRuntimeAnswer | null) ?? null}
          editable={editable}
          onChange={(a) => onChange(a)}
