...
export function DashboardTopNav({
  ...
  const base = `/${locale}`;
  // Kept in step with the desktop sidebar.
  const teacherItems = [
      ...
      <div className="flex h-16 items-center justify-between px-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            ...
            className="relative flex h-9 w-9 items-center justify-center rounded-xl bg-tint text-ink-strong"
          >
            <Bell className="h-4 w-4" />
            {notificationCount > 0 && (
              <span className="absolute -right-1 -top-1 flex h-4 min-w-[1rem] items-center justify-center rounded-full bg-red px-1 text-[9px] font-bold text-white">
