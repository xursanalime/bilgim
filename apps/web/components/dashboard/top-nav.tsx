...
export function DashboardTopNav({
  ...
  // Custom (non-Radix) drawer: trap focus while open, close on Esc, and
  // restore focus to the menu button when it closes (Req 7.1, 7.2, 7.3).
    ...
    <header className="sticky top-0 z-40 border-b border-rim bg-white lg:hidden">
      <div className="flex h-16 items-center justify-between px-6">
        <button
          ...
          className="flex h-11 w-11 items-center justify-center rounded-xl bg-tint text-ink-strong"
          aria-label="Menu"
        ...
        <div className="flex items-center gap-3">
          {role === 'STUDENT' && <XpBadge locale={locale} />}
      ...
      {mobileOpen && (
        <div
          ...
          aria-modal="true"
          aria-label="Menyu"
          className="fixed inset-0 top-16 z-[100] h-[calc(100vh-4rem)] w-full overflow-y-auto bg-white animate-in slide-in-from-top duration-300"
        >
