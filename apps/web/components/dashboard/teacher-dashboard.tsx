...
function Skeleton({ className }: { className?: string }) {
  return (
    <div className={`animate-pulse rounded-xl bg-tint ${className}`} />
  );
}
...
function KpiGrid({ data, isLoading, error, locale }: KpiGridProps) {
  const kpis = useMemo(
    () => [
      {
        label: 'Talabalar',
  ...
  return (
    <section aria-label="Asosiy koʻrsatkichlar">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {/* Each card already animated on hover — lifting, deepening its shadow,
            tinting its border — while being an inert <div>. Now the affordance
            is honest: every KPI opens the surface it summarises. */}
        {kpis.map((kpi) => (
          <Link
            key={kpi.label}
            href={kpi.href}
            className="group relative overflow-hidden rounded-3xl border border-rim bg-canvas p-5 shadow-soft transition-all hover:-translate-y-1 hover:shadow-medium hover:border-blue/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue/40"
          >
            ...
            <div className="mt-4">
              ...
              <div className="mt-0.5 flex items-center gap-1 text-[11px] font-medium text-ink-soft">
                {kpi.hint}
                <ArrowRight className="h-3 w-3 shrink-0 opacity-0 transition-all group-hover:translate-x-0.5 group-hover:opacity-100 group-hover:text-blue" />
              </div>
            </div>
          </Link>
        ))}
...
/**
 * One colour per status — every value must be distinct, otherwise adjacent
 * donut arcs merge into one and the legend stops explaining the chart.
 *
 *   DRAFT      grey  — not started
 *   SUBMITTED  blue  — waiting on the teacher (the actionable slice)
 *   IN_REVIEW  amber — in progress
 *   GRADED     teal  — done
 *   RETURNED   red   — sent back
 */
    ...
    <section className="flex h-full flex-col rounded-3xl border border-rim bg-canvas p-6 shadow-soft">
      <div className="flex items-center justify-between">
        ...
        <Link
          ...
          className="group flex shrink-0 items-center gap-1 rounded-full border border-rim px-3 py-1.5 text-[11px] font-bold text-ink-soft transition-all hover:border-blue/30 hover:text-blue"
        >
          Barchasi
