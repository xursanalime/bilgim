      ...
      <header className="group relative overflow-hidden rounded-[2rem] border border-rim bg-white p-5 shadow-soft sm:rounded-[2.5rem] sm:p-10">
        ...
        <div className="relative z-10 flex flex-col gap-6 sm:flex-row sm:items-center">
          <div className="relative self-center sm:self-auto">
            ...
            <button className="absolute -bottom-2 -right-2 flex h-7 w-7 items-center justify-center rounded-lg bg-blue text-white shadow-sm transition-transform hover:scale-110 active:scale-95 border-2 border-white sm:h-8 sm:w-8 sm:rounded-xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
        <section className="overflow-hidden rounded-[2rem] border border-rim bg-white shadow-soft sm:rounded-[2.5rem]">
          ...
          <div className="p-6 space-y-6 sm:p-8">
            ...
            {/* Bio — only TEACHER/STUDENT have a profile row to persist it in;
                for ADMIN the backend has nowhere to save this, so the field
                is hidden rather than showing a false "saved" success. */}
            {role !== 'ADMIN' && (
