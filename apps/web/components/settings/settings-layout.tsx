...
export function SettingsLayoutClient({ locale }: SettingsLayoutClientProps) {
  const t = useTranslations('settings.tabs');
  const pathname = usePathname();
  const base = `/${locale}/settings`;

  // Resolve the role after mount so SSR and the first client render match
  // (the token lives in a cookie/localStorage, unavailable during SSR).
  ...
  const tabs = [
    ...
    // Subscription management is instructor-only (Req 13).
    ...(role === 'TEACHER'
      ? [{ href: `${base}/subscription`, label: t('subscription'), icon: Sparkles }]
      : []),
    { href: `${base}/billing`, label: t('billing'), icon: CreditCard },
  ];
