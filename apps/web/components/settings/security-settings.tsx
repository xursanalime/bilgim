...
import { MfaSettings } from '../auth/mfa/mfa-settings';
...
/**
 * Account security surface: password change + MFA management (TOTP enrollment,
 * passkeys/WebAuthn, and backup codes). Rendered by the server `security` page
 * which supplies the authenticated user's role and locale.
 */
export function SecuritySettings({ locale, role }: SecuritySettingsProps) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 sm:space-y-10">
      ...
      <MfaSettings locale={locale} role={role} />
    </div>
  );
}
