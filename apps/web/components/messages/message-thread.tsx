...
import { mediaApi, type MediaAssetMetadata, type MediaPlaybackResponse } from '../../lib/api/media';
...
import { ProtectedImage, ProtectedAudio, ProtectedLightbox } from '../ui/protected-media';
...
export function MessageThread({ threadId, lessonId }: MessageThreadProps): React.ReactElement {
  ...
  // Custom (non-Radix) attachment popover: trap focus, close on Esc, and
  // restore focus to the trigger on close (Req 7.1–7.3).
  useFocusTrap(attachmentMenuRef, {
  ...
  }, [threadId, messages.length, messagesQuery.isLoading, messagesQuery.isError, queryClient]);
  ...
  // Real-time transport (join room, append inbound messages, keep the
  // nav unread badge + conversation list in sync) lives in a shared
  // hook built on `useRealtime` (Task 10.2, Req 14.3/14.4).
  useDmRealtime({
    threadId,
  ...
  async function startUpload(file: File, optId: string) {
    try {
      const result = await uploadFile(file, getMediaKind(file), (percent) => {
      ...
      <header className="flex items-center justify-between border-b border-rim bg-white/80 px-6 py-4 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-xl text-ink-faint hover:bg-tint hover:text-ink-strong transition-colors"
          >
            <Search className="h-4.5 w-4.5" />
          </button>
          <button
            type="button"
            aria-label="Suhbat sozlamalari"
            className="flex h-10 w-10 items-center justify-center rounded-xl text-ink-faint hover:bg-tint hover:text-ink-strong transition-colors"
          >
            ...
            <div className="relative shrink-0">
              ...
              {attachmentMenuOpen && (
                <div
                  ...
                  role="menu"
                  aria-label="Fayl turini tanlang"
                  className="absolute bottom-14 left-0 z-30 w-48 overflow-hidden rounded-2xl border border-rim bg-white p-1.5 shadow-xl"
                >
                  <button
                    type="button"
                    role="menuitem"
...
function MessageBubble({
  ...
  const [playback, setPlayback] = useState<{ url: string; type: string } | null>(
    (message as any).assetUrl ? { url: (message as any).assetUrl, type: 'original' } : null
  );
  const [isLightboxOpen, setIsLightboxOpen] = useState(false);

  useEffect(() => {
    if (message.assetId) {
      mediaApi.getAssetMetadata(message.assetId).then(setAssetMeta).catch(console.error);

      if (!(message as any).assetUrl) {
        mediaApi.getPlaybackUrl(message.assetId).then(setPlayback).catch(console.error);
      }
    }
  }, [message.assetId, (message as any).assetUrl]);
    ...
    <div className={cn(
      ...
      {!isMine && showAvatar && (
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-tint border border-rim shadow-sm overflow-hidden">
          {peerAvatar ? (
            <img src={peerAvatar} alt="" className="h-full w-full object-cover" draggable={false} onContextMenu={(e) => e.preventDefault()} />
          ) : peerName ? (
      ...
      <div className={cn(
        ...
        >
          ...
          {optimistic && (
            <div className="relative mb-2 overflow-hidden rounded-xl border border-white/10 shadow-sm min-w-[200px] min-h-[150px] bg-ink-strong/5 flex items-center justify-center">
              {isImage && (
              ...
              {optimistic.status !== 'completed' && (
                <button
                  type="button"
                  onClick={onCancel}
                  aria-label="Yuklashni bekor qilish"
                  className="absolute top-2 right-2 flex h-6 w-6 items-center justify-center rounded-full bg-ink-strong/50 text-white hover:bg-ink-strong/80 transition-colors"
                >
          ...
          {/* Real Media Content */}
          {!optimistic && assetMeta && (message.assetId) && (
            <div className="mb-2 space-y-2">
              {/* RASM: student uchun himoyali ProtectedImage, teacher uchun oddiy */}
              {assetMeta.kind === 'IMAGE' && (
                isStudent ? (
                  <ProtectedImage
                    assetId={message.assetId}
                    alt={assetMeta.fileName}
                    className="max-h-60 w-full rounded-xl border border-white/10 shadow-sm overflow-hidden"
                    onClick={() => setIsLightboxOpen(true)}
                  />
                ) : (
                  <button
                    type="button"
                    aria-label={`${assetMeta.fileName} — kattalashtirish`}
                    className="block w-full cursor-zoom-in overflow-hidden rounded-xl border border-white/10 shadow-sm"
                    onClick={() => setIsLightboxOpen(true)}
                  >
                    {playback && <img src={playback.url} alt={assetMeta.fileName} className="max-h-60 w-full object-cover" />}
                  </button>
                )
              )}

              {/* VIDEO */}
              {assetMeta.kind === 'VIDEO' && playback && (
              ...
              {/* AUDIO: student uchun himoyali ProtectedAudio */}
              ...
              {/* FAYL (PDF, DOC, SHEET, OTHER) */}
              {(assetMeta.kind === 'PDF' || assetMeta.kind === 'DOC' || assetMeta.kind === 'SHEET' || assetMeta.kind === 'OTHER') && (
                <div className={cn(
                  ...
                  {/* Faqat teacher yuklab olishi mumkin */}
                  {!isStudent && playback ? (
                    <a
                      href={playback.url}
                      ...
                      aria-label={`${assetMeta.fileName} faylini yuklab olish`}
                      className={cn(
                    ...
                    <span className="text-sm opacity-40" title="Yuklab olish taqiqlangan">🔒</span>
                  )}
                </div>
              )}
            </div>
      ...
      {isLightboxOpen && message.assetId && (
        isStudent ? (
          <ProtectedLightbox
            assetId={message.assetId}
            alt={assetMeta?.fileName}
...
function describeUploadError(error: unknown): string {
  if (error instanceof ApiClientError) {
    ...
    if (details?.code === 'MEDIA_CONTENT_TYPE_NOT_ALLOWED') {
