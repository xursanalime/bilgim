...
export function GroupChatThread({
  ...
  // Messages are newest-first, so `messages[0]` is the latest one. Keying
  // the effect on its id (rather than `messages.length`) means this only
  // fires when a genuinely new message arrives — scrolling back to load
  // older history changes `.length` too but never touches `messages[0]`,
  // so it no longer re-fires the PATCH on every page of loaded history.
  const latestMessageId = messages[0]?.id ?? null;
  useEffect(() => {
    if (!groupId || !latestMessageId || messagesQuery.isLoading || messagesQuery.isError) {
      return;
    }
    groupChatApi.markRead(groupId).then(() => {
      queryClient.invalidateQueries({ queryKey: ['group-chat', 'groups'] });
    });
  }, [groupId, latestMessageId, messagesQuery.isLoading, messagesQuery.isError, queryClient]);
  ...
  async function startUpload(file: File, optId: string) {
    try {
      const assetId = await uploadFile(file, (percent) => {
  ...
  return (
    <div className={cn('group flex w-full gap-3', isMine ? 'flex-row-reverse' : 'flex-row')}>
      ...
      <div className={cn('flex max-w-[80%] flex-col gap-1', isMine ? 'items-end' : 'items-start')}>
        ...
        >
          {optimistic && (
            <div className="relative mb-2 flex min-h-[150px] min-w-[200px] items-center justify-center overflow-hidden rounded-xl border border-white/10 bg-tint-strong/5 shadow-sm">
              {isImage && (
              ...
              {optimistic.status !== 'completed' && (
                <button
                  onClick={onCancel}
                  className="absolute right-2 top-2 flex h-6 w-6 items-center justify-center rounded-full bg-tint-strong/50 text-white transition-colors hover:bg-tint-strong/80"
                >
