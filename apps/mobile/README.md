...
Bilgim React Native (Expo SDK 51) mobil ilovasi. Phase 8 (`task 22`)
...
- **`@bilgim/api-client`** — workspace ichidagi shared HTTP client
  (web va mobile uchun bitta transport, refresh dedup, error envelope)
- **`@bilgim/i18n`** — uz / ru / en lokalizatsiya bundle (web bilan
  bir xil)
...
```bash
pnpm --filter @bilgim/mobile dev         # = expo start
pnpm --filter @bilgim/mobile start       # = expo start
pnpm --filter @bilgim/mobile ios         # iOS simulator
pnpm --filter @bilgim/mobile android     # Android emulator
pnpm --filter @bilgim/mobile web         # Browserda preview
pnpm --filter @bilgim/mobile typecheck   # tsc --noEmit
```
...
│   ├── api-client.ts          # @bilgim/api-client SDK instance + storage adapter wiring
│   ├── api/
...
2. `useAuth().login(email, password)` `sdk.auth.login()` ni chaqiradi
   (shared `@bilgim/api-client`). U `POST /api/v1/auth/login` ga
...
`@bilgim/api-client` 401 javobida bitta `POST /auth/refresh`
...
`app.json#scheme = bilgim` orqali backend mailida yuborilgan
`https://bilgim.uz/uz/verify-email?token=…` URL ga `bilgim://verify-email?token=…`
...
`bilgim.uz` leaf certificate's SubjectPublicKeyInfo (SPKI) so a
...
Then `pnpm --filter @bilgim/mobile expo prebuild --clean` rolls
...
3. **Expo push token**ni olamiz (`lib/notifications/token.ts`) va
   ...
   `expo-secure-store` ichida `bilgim_push_token` kalitida saqlanadi.
