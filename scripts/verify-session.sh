...
code=$(curl -s -o /dev/null -w "%{http_code}" -H "Cookie: edubridge_access_token=$ACCESS; edubridge_refresh_token=$REFRESH" "$WEB/uz/dashboard")
echo "HTTP $code"
...
HDRS=$(curl -s -D - -o /dev/null -H "Cookie: edubridge_access_token=$EXPIRED; edubridge_refresh_token=$REFRESH" "$WEB/uz/dashboard")
echo "$HDRS" | head -1
echo "$HDRS" | grep -i "location:" || echo "  (no redirect — session refreshed)"
echo "$HDRS" | grep -i "set-cookie: edubridge_access_token" >/dev/null && echo "  ✓ new access cookie set by middleware" || echo "  ✗ no refreshed cookie"
