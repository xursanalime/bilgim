...
MAILHOG_BIN="$LOCAL_DIR/bin/mailhog"
MAILHOG_SMTP_PORT="1025"
MAILHOG_UI_PORT="8025"
MAILHOG_PID_FILE="$LOCAL_DIR/mailhog.pid"
...
stop_stack() {
  ...
  if [ -f "$MAILHOG_PID_FILE" ]; then
    kill "$(cat "$MAILHOG_PID_FILE")" >/dev/null 2>&1 || true
...
# ---- MailHog (SMTP sink + web inbox) ----
# The API sends every transactional mail (email verification, password
# reset, notification digests) through SMTP_HOST:SMTP_PORT, which .env
# points at localhost:1025. Without a listener there the outbox email
# worker retries and dead-letters, and — because the verification token
# is stored only as a SHA-256 hash — nobody can finish registration
# locally. MailHog accepts everything and shows it at :8025.
if ! curl -s -o /dev/null -m 2 "http://127.0.0.1:${MAILHOG_UI_PORT}/" 2>/dev/null; then
  ...
  echo "==> Starting MailHog on SMTP $MAILHOG_SMTP_PORT / UI $MAILHOG_UI_PORT..."
  nohup "$MAILHOG_BIN" \
  ...
  for _ in $(seq 1 30); do
    curl -s -o /dev/null -m 1 "http://127.0.0.1:${MAILHOG_UI_PORT}/" 2>/dev/null && break
    sleep 0.5
  done
else
...
echo "   API:   http://localhost:${API_PORT:-4000}"
echo "   Web:   http://localhost:${WEB_PORT:-3000}"
echo "   Mail:  http://localhost:${MAILHOG_UI_PORT}  (verification links land here)"
