...
Shared HTTP client for the Bilgim web and mobile apps.

This package wraps `fetch` with the conventions every Bilgim frontend
needs:
...
- `StorageKeys.ACCESS_TOKEN` (`bilgim_access_token`)
- `StorageKeys.REFRESH_TOKEN` (`bilgim_refresh_token`)
- `StorageKeys.USER` (`bilgim_user`)
