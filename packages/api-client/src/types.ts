 ...
 * (e.g. full DTO shapes) belongs in `@bilgim/shared-types`.
 *
 * We deliberately keep the role union local rather than importing the
 * `UserRole` enum from `@bilgim/shared-types` so this package has
 ...
 * `@bilgim/shared-types` `UserRole` so client and server stay
