# Infrastructure Hardening & GitOps — EduBridge Platform

Bu hujjat EduBridge platformasining infratuzilma xavfsizligi va GitOps strategiyasini tavsiflaydi.

## Umumiy ko'rinish

EduBridge production infratuzilmasi quyidagi tamoyillar asosida qurilgan:
...
- **Repository:** `github.com/edubridge-uz/edubridge-platform`
...
|---|---|---|---|
| `edubridge-production` | `main` | `infra/k8s/overlays/production` | `edubridge-production` |
| `edubridge-staging` | `develop` | `infra/k8s/overlays/staging` | `edubridge-staging` |
| `edubridge-infrastructure` | `main` | `infra/k8s/base` | `edubridge-production` |
...
### Vault arxitekturasi
```
Vault Server (edubridge-vault namespace)
├── Secret Engine: KV v2 (static secrets)
│   ├── secret/data/edubridge/app (JWT, API keys)
│   ├── secret/data/edubridge/database (DB credentials)
│   ├── secret/data/edubridge/redis (Redis password)
│   ├── secret/data/edubridge/cloudflare (R2 keys)
│   ├── secret/data/edubridge/email (SMTP credentials)
│   └── secret/data/edubridge/telegram (Bot token)
├── Secret Engine: Database (dynamic credentials)
│   └── database/creds/edubridge-app (auto-rotated PostgreSQL creds)
├── Secret Engine: Transit (envelope encryption)
│   └── transit/keys/edubridge-dek (KMS for data encryption)
└── Secret Engine: PKI (TLS certificates)
    └── pki/issue/edubridge (auto-renewed TLS certs)
```
...
2. **ValidatingAdmissionPolicy** (ikkilamchi):
   - `edubridge-deny-privileged` — privileged, hostNetwork, hostPID, hostIPC
   - `edubridge-enforce-nonroot` — runAsNonRoot, allowPrivilegeEscalation, capabilities
   - `edubridge-readonly-rootfs` — readOnlyRootFilesystem
   - `edubridge-deny-hostpath` — hostPath volumes
   - `edubridge-enforce-seccomp` — seccomp RuntimeDefault
...
vault kv put secret/edubridge/app new_key="value"
...
kubectl create job --from=cronjob/drift-detection manual-drift-check \
  -n edubridge-production
kubectl logs -f job/manual-drift-check -n edubridge-production
```
