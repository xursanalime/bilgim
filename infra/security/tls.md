...
This document describes the **TLS 1.3-only** posture EduBridge enforces
...
production zone (`edubridge.uz`) as follows. Settings live under
...
resource "cloudflare_zone_settings_override" "edubridge" {
  zone_id = var.zone_id
  settings {
    min_tls_version            = "1.3"
    tls_1_3                    = "on"
...
resource "cloudflare_zone_settings_override" "edubridge_hsts" {
  zone_id = var.zone_id
  settings {
    security_header {
      enabled            = true
...
API clients pin the SHA-256 fingerprints of the `edubridge.uz`
certificate chain.
