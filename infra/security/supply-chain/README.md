# Supply Chain Security — EduBridge Platform

> Bu hujjat EduBridge platformasining dasturiy ta'minot yetkazib berish zanjiri xavfsizligi
...
bosqichlarni himoyalash strategiyasi. EduBridge quyidagi himoya qatlamlarini qo'llaydi:
...
- `edubridge` namespace `policy.sigstore.dev/include=true` label bilan belgilanadi
...
kubectl label ns edubridge policy.sigstore.dev/include=true
...
```bash
# Lokal Trivy scan
trivy image edubridge-api:dev

# Lokal SBOM generatsiya
syft scan edubridge-api:dev -o cyclonedx-json > sbom-local.json
